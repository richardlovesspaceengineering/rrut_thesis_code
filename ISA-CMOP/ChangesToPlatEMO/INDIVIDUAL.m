...
%--------------------------------------------------------------------------
    ...
    properties(SetAccess = private)
        ...
        mcv;        % mean constraint violations of the individual
        normcv;     % constraint violations norm of the individual
        cvr;        % Constraint violations rate
    ...
    methods
        ...
        %       H = INDIVIDUAL(rand(100,10),randn(100,3))
            ...
            if nargin > 0
                ...
                for i = 1 : length(obj)
                    obj(i).dec = Decs(i,:);
                    obj(i).obj = Objs(i,:);
                    obj(i).con = Cons(i,:); obj(i).con(obj(i).con<0) =0;
                    obj(i).mcv = sum(max(0,obj(i).con))/numel(obj(i).con); %added by me and its fun
                    obj(i).normcv = norm(obj(i).con); %added by me and its fun
                    obj(i).cvr = numel(find(all(obj(i).con>0))); %added by me and its fun
                end
                if nargin > 1
                    for i = 1 : length(obj)
        ...
        %   population obj, where obj is an array of INDIVIDUAL objects.
         ...
         %% Get the matrix of mean constraint violations of the population
        function value = mcvs(obj)
        %cons - Get the matrix of constraint violations of the population.
        %
        %   A = obj.cons returns the matrix of average constraint violations of the
        %   population obj, where obj is an array of INDIVIDUAL objects.
        
            value = cat(1,obj.mcv);
        end
        %% Get the matrix of constraint violations norms of the population
        function value = normcvs(obj)
        %cons - Get the matrix of constraint violations of the population.
        %
        %   A = obj.cons returns the matrix of average constraint violations of the
        %   population obj, where obj is an array of INDIVIDUAL objects.
        
            value = cat(1,obj.normcv);
        end
        %% Get the matrix of constraint violations rate of the population
        function value = cvrs(obj)
        %cons - Get the matrix of constraint violations of the population.
        %
        %   A = obj.cons returns the matrix of average constraint violations of the
        %   population obj, where obj is an array of INDIVIDUAL objects.
