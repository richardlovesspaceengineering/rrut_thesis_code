...
class PinterSetup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.n_var, 'c',
                           lower=-2.0*np.ones(self.n_var), upper=3.0*np.ones(self.n_var),
    ...
    def obj_func_specific(self, x):
        ...
        eqn1 = x1 - np.sin(2 * x1 + 3 * y1) - np.cos(3 * x1 - 5 * y1)
        eqn2 = y1 - np.sin(x1 - 2 * y1) + np.cos(x1 + 3 * y1)
