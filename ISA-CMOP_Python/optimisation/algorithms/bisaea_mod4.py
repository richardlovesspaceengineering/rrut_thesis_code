...
class BISAEA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.history_count = []
        self.clusters, self.d1_mat, self.d2_mat = None, None, None     # APD/ Theta parameters
    ...
    def infill_selection(self, population, n_survive=1):
        ...
        # Assign APD clusters to offspring
        self.clusters, self.d1_mat, self.angle_mat, self.gamma_apd = apd_association(obj_array, self.ref_dirs,
                                                                                     self.ideal)
        best_per_cluster, best_reps = self.best_from_cluster(flag='apd')

        # Select best point for given cluster
        selected_weight = self.ref_dirs[self.last_cluster]
        idx = best_per_cluster[self.last_cluster]
        if len(idx) == 0:
            print(colored('Used Weighted Sum!!', 'red'))
        ...
        print(colored('Selected Cluster: ', 'blue'), self.last_cluster,
              colored('Ref Dir: ', 'blue'), selected_weight)
    ...
    def best_from_cluster(self, flag='apd'):
