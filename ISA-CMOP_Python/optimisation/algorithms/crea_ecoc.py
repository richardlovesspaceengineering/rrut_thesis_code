...
class CREA(SAEvolutionaryAlgorithm):
    ...
    def initialise_classifier(self, flag=None, n_max_class=4):
        n_max_class = len(self.ref_dirs)
        ...
        # Extract vars and objs from population
        population = copy.deepcopy(self.population)
        var_array = copy.deepcopy(population.extract_var())
        obj_array = copy.deepcopy(population.extract_obj())

        # Pareto-ranking (convergence)
        if 'dom1' in flag:
            fronts = NonDominatedSorting().do(obj_array, return_rank=False)

        # Reference vector assignment (diversity)
        else:
            self.update_clusters(flag=self.dom2_flag)
            fronts = [np.array(item) for item in self.clusters]
            print(self.clusters)
        ...
        for i in range(len(fronts)):
            # classify only first 3 ranks
            if counter == n_max_class:
                break

            # Determine indices corresponding to next rank
            selected_front = fronts[i]

            # if more than 1 individual in the ranked class
            if len(selected_front) > 2:
                train_x = var_array[selected_front]
                train_y = current_rank * np.ones(len(train_x))

                # Append to training set
                if training_data[0] is None:
                    training_data[0] = train_x
                    training_data[1] = train_y
                else:
                    training_data[0] = np.vstack((training_data[0], train_x))
                    training_data[1] = np.hstack((training_data[1], train_y))
                current_rank += 1
            counter += 1

        # Conduct SMOTE Over-sampling (to balance training)
        oversampler = SMOTE(k_neighbors=2)
        training_data[0], training_data[1] = oversampler.fit_resample(training_data[0], training_data[1])
        ...
        # if 'dom2' in flag:
        #     classifier = OneVsOneClassifier(estimator=KNeighborsClassifier(n_neighbors=3))
    ...
    def infill_selection(self, offspring, population, n_survive=1, counter='seq_perm'):
        ...
        # Assign predicted ranks to offspring
        ranks = []
        off_vars = np.atleast_2d(offspring.extract_var())
        for classifier in self.classifier_surrogates:
            ranks.append(classifier.predict(off_vars))
        ...
        scaling = 1.0
        origin = np.zeros(len(self.ref_dirs))
        x_vec = scaling * np.vstack((origin, self.ref_dirs[:, 0])).T
        y_vec = scaling * np.vstack((origin, self.ref_dirs[:, 1])).T
        ...
        for idx, rank in enumerate(np.unique(ranks[1])):
            indices = ranks[1] == rank
            offs = np.atleast_2d(offspring[indices].extract_obj())
            ax.scatter(offs[:, 0], offs[:, 1], color=line_colors[idx], s=150, label=str(rank))
