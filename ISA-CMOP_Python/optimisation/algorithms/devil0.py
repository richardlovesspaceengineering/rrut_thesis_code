...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        # if self.evaluator.n_eval % 200 == 0:
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            mod_offspring.assign_obj(-mod_offspring.extract_obj())
            selected = self.rank_and_crowd._do(self.problem, offspring, n_survive=int(n_offspring / 5), cons_val=mod_offspring.extract_cons_sum())
            ...
            # best_front = self.nondom.do(offspring.extract_obj(), cons_val=cons_sum,
