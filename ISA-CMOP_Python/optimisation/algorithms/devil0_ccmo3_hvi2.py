...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def probabilistic_infill_selection(self, offspring, n_survive=1):
        ...
        probabilities, offspring = self.calculate_hypervolume_improvement_and_angle_diversity(offspring, self.opt)
    ...
    def calculate_hypervolume_improvement_and_angle_diversity(self, population, opt):
        # Extract objectives
        opt_obj = np.atleast_2d(opt.extract_obj())
        pop_obj = np.atleast_2d(population.extract_obj())

        # Determine reference point
        nadir = np.max(opt_obj, axis=0)
        nadir += 0.2 * np.abs(nadir)

        # Remove individuals that are worse than the reference point
        not_worse_than = np.all(pop_obj <= nadir, axis=1)
        if np.count_nonzero(not_worse_than) > 0:
            pop_obj = pop_obj[not_worse_than]
        else:
            # calculate nadir point from merged population
            merged_obj_arr = np.vstack((opt_obj, pop_obj))
            nadir = np.max(merged_obj_arr, axis=0)
            nadir += 0.2 * np.abs(nadir)
            not_worse_than[:] = True
            print('Defaulting to total pop hvi calculation!')
        ...
        min_angle = self.predict_angle_diversity(population[not_worse_than], comparison_population)

        # Combine the two metrics together
        hvi = (hvi_arr - np.min(hvi_arr)) / (np.max(hvi_arr) - np.min(hvi_arr))
        probabilities = 2 * hvi + min_angle

        return probabilities, population[not_worse_than]
