...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        cv_sum = self.calc_cv(self.population)
        lowest_cv = np.argsort(cv_sum)[:2]
        pop_arr = self.population.extract_var()
        arr1 = pop_arr[lowest_cv[0]]
        arr2 = pop_arr[lowest_cv[1]]
        dir = arr1 - arr2
        arr3 = arr2 + 2 * dir
        new_points = np.vstack((arr1, arr3))
        ...
        print(f'LHS dist: {100*np.linalg.norm(new_ub - new_lb) / np.linalg.norm(self.problem.x_upper - self.problem.x_lower):.3f} %')
