...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def generate_offspring(self, representatives, n_offspring, frac=1.0):
        ...
        # TODO: Seed an LHS around the two lowest CV points
        prob_dist = np.linalg.norm(self.problem.x_upper - self.problem.x_lower)
        dist = 0.5 * (frac / 100) * prob_dist / np.sqrt(self.problem.n_var)

        cv_sum = self.calc_cv(self.population)
        lowest_cv = np.argsort(cv_sum)[:1]
        pop_arr = self.population.extract_var()
        arr1 = pop_arr[lowest_cv]
        arr2 = arr1 + dist
        arr3 = arr1 - dist
        new_points = np.vstack((arr2, arr3))
        ...
        # # TODO: debug plotting ----------------
        # if self.evaluator.n_eval > 200:
        #     eucd_dist = -distance.cdist(np.atleast_2d(arr1), pop_arr).flatten()
        #     new_obj_arr = np.hstack((eucd_dist[:, None], cv_sum[:, None]))
        #     best_front = self.nondom.do(new_obj_arr, only_non_dominated_front=True)
        #     nondom_arr = new_obj_arr[best_front]
        #     fig, ax = plt.subplots(1, 1, figsize=(9, 7))
        #     ax.scatter(new_obj_arr[:, 0], new_obj_arr[:, 1], color='k', label="Population")
        #     ax.scatter(nondom_arr[:, 0], nondom_arr[:, 1], color='r', label="Non-dominated Parents")
        #     ax.set_ylabel("CV")
        #     ax.set_xlabel("Euclidean Dist. from lowest CV individual")
        #     plt.legend()
        #     plt.show()

        #     pop_obj_arr = self.population.extract_obj()
        #     eucd_dist = -distance.cdist(np.atleast_2d(pop_obj_arr[lowest_cv]), pop_obj_arr).flatten()
        #     new_obj_arr = np.hstack((eucd_dist[:, None], cv_sum[:, None]))
        #     best_front = self.nondom.do(new_obj_arr, only_non_dominated_front=True)
        #     nondom_arr = new_obj_arr[best_front]
        #     fig, ax = plt.subplots(1, 1, figsize=(9, 7))
        #     ax.scatter(new_obj_arr[:, 0], new_obj_arr[:, 1], color='k')
        #     ax.scatter(nondom_arr[:, 0], nondom_arr[:, 1], color='r')
        #     # plt.show()
        #
        #     opt_obj_arr = self.opt.extract_obj()
        #     fig, ax = plt.subplots(1, 1, figsize=(9, 7))
        #     ax.scatter(pop_obj_arr[:, 0], pop_obj_arr[:, 1], color='k')
        #     ax.scatter(pop_obj_arr[best_front, 0], pop_obj_arr[best_front, 1], color='r')
        #     ax.scatter(opt_obj_arr[:, 0], opt_obj_arr[:, 1], color='g')
        #     plt.show()
        #
        ...
        lhs.do(n_offspring, new_lb, new_ub, np.random.randint(0, 1e6))
        x_lhs = lhs.x
        lhs_pop = Population(self.problem, len(x_lhs))
        lhs_pop.assign_var(self.problem, x_lhs)
        lhs_pop = self.surrogate_evaluator.do(self.surrogate.obj_func, self.problem, lhs_pop)
        survived = Population.merge(survived, lhs_pop)
    ...
    def attempt_solution_repair(self, infill, opt):
        ...
        x_repaired = self.repair_solution(infill, eps=1e-6, n_perturb=100 * self.problem.n_var, perturb_scale=0.1)  # eps=1e-3, n_perturb=1000, perturb_scale=3
    ...
    def repair_solution(self, infill, eps=1e-4, perturb_scale=3.0, n_perturb=1000):
