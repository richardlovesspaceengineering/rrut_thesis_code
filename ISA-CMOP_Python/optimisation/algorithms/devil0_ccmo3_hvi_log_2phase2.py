...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        if self.feasible_frac == 0.0:
            # INFEASIBLE STAGE =========================================================================================
            print("Using DCV stage")
            # Internal Offspring Generation
            self.offspring = self.generate_dcv_offspring(self.reps, n_offspring=self.n_offspring)

            # Probabilistic Infill Point Selection
            infill = self.dcv_infill_selection(self.offspring, n_survive=self.n_infill)
            # infill = self.probabilistic_infill_selection(self.offspring, n_survive=self.n_infill)
        else:
            # FEASIBLE STAGE ===========================================================================================
            print("Using CCMO3 stage")
            # Internal Offspring Generation
    ...
    def generate_dcv_offspring(self, representatives, n_offspring, frac=0.5):
        ...
        # Seed local points around lowest CV violated individual in population
        seed_population = self.calc_seed_pop(lowest_cv=self.lowest_cv, n_points=n_offspring, frac=frac)
        survived = Population.merge(survived, seed_population)
        ...
        for gen in range(self.w):
            ...
            # dcv_obj_arr = self.calc_dcv(offspring, lowest_cv=self.lowest_cv)
        ...
        # dcv_survived = self.calc_dcv(survived, lowest_cv=lowest_cv)
        # self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Pareto', 'Reps', 'Surv'],
        #                  obj_array2=dcv_obj_arr,
        #                  obj_array3=dcv_survived)
    ...
    def dcv_infill_selection(self, offspring, n_survive=1):
        ...
        # Determine DCV for opt and offspring population
        dcv_offs = self.calc_dcv(offspring, lowest_cv=self.lowest_cv)
        dcv_opt = self.calc_dcv(self.reps, lowest_cv=self.lowest_cv)

        # Calculate Hypervolume Improvement of each offspring
        hvi = self.calculate_dcv_hypervolume_improvement(dcv_offs, dcv_opt)
    ...
    def generate_ga_de_offspring(self, survived, n_offspring, evaluate=True):
        ...
        # offspring = Population.merge(offspring, survived)  # TODO: Fix all problems?
    ...
    def probabilistic_infill_selection(self, offspring, n_survive=1):
        ...
        comparison_population = Population.merge(copy.deepcopy(self.reps), copy.deepcopy(self.infilled_pop[-10:]))
    ...
    def calculate_dcv_hypervolume_improvement(self, pop_obj, opt_obj):
        # Determine reference point
        merged_obj_arr = np.vstack((opt_obj, pop_obj))
        nadir = np.max(merged_obj_arr, axis=0)
        nadir += 0.2 * np.abs(nadir)
    ...
    def reps_survival_mechanism(self, population):
        ...
        if feasible_frac == 0.0 or len(survived) < self.problem.n_var:
            ...
            best_front = self.nondom.do(dcv_obj_arr, only_non_dominated_front=True)
            best_front = self.rank_and_crowd._do(self.problem, dummy_pop, n_survive=max(self.problem.n_var, len(best_front)), cons_val=None)
