...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.n_infill = 1
        self.n_offspring = 30 * self.problem.n_var
        self.n_surv = 6 * self.problem.n_var
    ...
    def _next(self):
        ...
        self.offspring = self.generate_offspring(self.opt, n_offspring=self.n_offspring, n_survive=self.n_surv)
    ...
    def generate_offspring(self, representatives, n_offspring, n_survive):
        ...
        for gen in range(self.w):
            ...
            if self.last_selected == 0:
                # print('rcrowd')
                survived = self.rank_and_crowd.do(self.problem, offspring, n_survive=n_survive)
            elif self.last_selected == 1:
                # print('thor')
                survived = self.two_rank_survival.do(self.problem, offspring, n_survive=n_survive)
                ...
                selected = self.safr_survival._do(self.problem, eps_population, n_survive, cons_val=eps_population.extract_cons_sum())
    ...
    def survival_mechanism(self, population, filter_feasible=True):
        ...
        if filter_feasible:
            ...
            feasible_frac = n_feasible / len(population)
            if feasible_frac > 0.0:
                n_survive = min(n_feasible, self.n_opt)
                survived = self.survival.do(self.problem, population, n_survive, gen=self.n_gen, max_gen=self.max_gen)
