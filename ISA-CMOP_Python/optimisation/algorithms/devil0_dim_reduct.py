...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.n_var_original = copy.deepcopy(self.problem.n_var)
        self.n_var_reduced = self.problem.n_var
        self.pca = None
        self.population_reduced = None
        ...
        self.train_pca(self.population, variance_frac=0.85)
        self.train_regressors_reduced(self.population)
    ...
    @staticmethod
    def calculate_hypervolume_improvement(population, opt):
        # Extract objectives
    ...
    def train_pca(self, population, variance_frac=0.85):
        # Normalise input variables
        x_vars = population.extract_var()
        x_norm = (x_vars - self.problem.x_lower) / (self.problem.x_upper - self.problem.x_lower)
        self.pca = PCA(n_components=variance_frac).fit(x_norm)
        self.n_var_reduced = self.pca.n_components_

        # Transform new training data
        self.problem.n_var = self.n_var_reduced
        self.population_reduced = Population(self.problem, len(self.population))
