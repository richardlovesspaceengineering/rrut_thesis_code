...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            # # TODO: minimise constraint selection
            ...
            # cons_arr = offspring.extract_cons()
            # obj_1 = cons_arr[:, self.active_cons]
            # obj_2 = copy.deepcopy(cons_arr[:, self.inactive_cons])
            # obj_2[obj_2 <= 0.0] = 0.0
            # obj_2 = np.sum(obj_2, axis=1)
            # obj_arr = np.hstack((obj_1, obj_2[:, None]))
