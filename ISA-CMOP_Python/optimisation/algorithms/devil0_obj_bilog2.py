...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        bilog_obj_arr = bilog_transform(copy.deepcopy(self.population.extract_obj()), beta=1)
        bilog_obj_arr[:, 0] = self.population.extract_obj()[:, 0]
        for obj_cntr, model in enumerate(self.surrogate.obj_surrogates):
            model.reset()
            model.add_points(self.population.extract_var(), bilog_obj_arr[:, obj_cntr].flatten())
            model.train()
        ...
        # obj_arr = self.population.extract_cons()
        # print((np.max(obj_arr, axis=0) - np.min(obj_arr, axis=0)) / np.std(obj_arr, axis=0))
