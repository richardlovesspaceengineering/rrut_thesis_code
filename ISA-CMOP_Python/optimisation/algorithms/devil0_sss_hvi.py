...
from optimisation.operators.survival.eps_relaxed_survival import EpsRelaxedSurvival
...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        # Surrogate Survival Switching (SSS)
        self.survival_list = [
            RankAndCrowdingSurvival(filter_infeasible=False),
            TwoRankingSurvival(filter_infeasible=False),
            SelfAdaptiveFeasibleRatioEpsilonSurvival(filter_infeasible=False),
            PopulationBasedEpsilonSurvival(filter_infeasible=False),
            EpsRelaxedSurvival(survival=SelfAdaptiveFeasibleRatioEpsilonSurvival(filter_infeasible=False),
                               evaluator=self.evaluator,
                               eps_frac=0.15, eps0=self.get_max_cons_viol(self.population), filter_infeasible=False)
        ]
        self.survival_name_list = ['r&crowd', 'tor', 'safr', 'popeps', 'eps-safr']
    ...
    def _next(self):
        ...
        self.last_selected += 1
        if self.last_selected == len(self.survival_list):
            self.last_selected = 0

        # Internal Offspring Generation
        print(f"Using survival CHT: {self.survival_name_list[self.last_selected]}")
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            # Select most promising individuals to survive by switching through different survival mechanisms
            survived = self.survival_list[self.last_selected].do(self.problem, offspring,
                                                                 n_survive=int(n_offspring / 5),
