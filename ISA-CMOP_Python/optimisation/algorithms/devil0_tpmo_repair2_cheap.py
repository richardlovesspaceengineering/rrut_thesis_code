...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        infill = self.attempt_solution_repair(infill, self.opt)
    ...
    def repair_solution(self, infill, eps=1e-4, perturb_scale=3, n_perturb=1000):
        ...
        for k in range(self.problem.n_con):
            ...
            grad = calculate_finite_difference_grad(self.problem, x_var, self.surrogate.cons_surrogates[k], eps=eps, k=k)
        ...
        for i in range(n_perturb):
            ...
            # TODO: Evaluate cheaply with real constraint functions
            cons = self.problem.cons_func_specific(x_candidates[i])
