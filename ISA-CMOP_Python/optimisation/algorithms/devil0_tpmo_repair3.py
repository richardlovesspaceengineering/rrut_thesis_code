...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        infill = self.attempt_infill_repair(infill, self.opt, cv_frac=0.1)

        # Update Archives
        self.population = Population.merge(self.population, infill)

        # TODO: Attempt to repair population with lowest CV and highest HVI over the Opt front
        if len(infill) == 1:
            infill2 = self.attempt_solution_repair(infill, self.opt, cv_frac=0.1)
            self.population = Population.merge(self.population, infill2)
            infill = Population.merge(infill, infill2)
        ...
        # Debugging Output
        cons_arr = copy.deepcopy(self.population.extract_cons())
        cons_arr[cons_arr <= 0.0] = 0.0
        cv_sum = np.sum(cons_arr, axis=1)
        print(colored(f"min CV: {np.min(cv_sum):.3f}\tIGD: {self.igd:.3f}\tObj: {infill.extract_obj()}\tCons: {infill.extract_cons()}\tFeas: {self.feasible_frac:.3f}", 'yellow'))
    ...
    def attempt_infill_repair(self, infill, opt, cv_frac=0.1):
        # Check feasibility (Only repair infill if cv < cv_frac)
        cons_arr = copy.deepcopy(infill.extract_cons())
        cons_arr[cons_arr <= 0.0] = 0.0
        cv = np.sum(cons_arr)
        if 0.0 < cv <= cv_frac:
            # Perform repair mechanism (RI-2 from SACOBRA)
            x_repaired = self.repair_solution(infill, eps=1e-3, n_perturb=100 * self.problem.n_var,
                                              perturb_scale=10)  # eps=1e-3, n_perturb=1000, perturb_scale=3
    ...
    def attempt_solution_repair(self, infill, opt, cv_frac=0.1):
        # Check feasibility (Only repair population individuals if a feasible point is found)
        cons_arr = copy.deepcopy(infill.extract_cons())
        cons_arr[cons_arr <= 0.0] = 0.0
        cv = np.sum(cons_arr)
        if cv > 0.0:
        ...
        # Determine which slightly-infeasible individuals in population dominate current front
        cons_arr = copy.deepcopy(self.population.extract_cons())
        cons_arr[cons_arr <= 0.0] = 0.0
        cv = np.sum(cons_arr, axis=1)
        almost_feasible = (cv < cv_frac) & (cv != 0.0)
        afeasible_arr = self.population.extract_obj()[almost_feasible]
        opt_arr = opt.extract_obj()
        merged_arr = np.vstack((afeasible_arr, opt_arr))
        best_front = self.nondom.do(merged_arr, only_non_dominated_front=True, return_rank=False)
        afeasible_indices = np.arange(len(afeasible_arr))
        dominating_points = np.in1d(afeasible_indices, best_front)
        to_repair = self.population[almost_feasible][dominating_points]
        if len(to_repair) == 0.0:
            # This means there are no points worth repairing
            return infill
        elif len(to_repair) > 1.0:
            # Select point with highest HVI score
            hvi = self.calculate_hypervolume_improvement(to_repair, opt)
            selected = np.array([np.argmax(hvi)])
            to_repair = to_repair[selected]
        old_cons = copy.deepcopy(to_repair.extract_cons())
        old_cons[old_cons <= 0.0] = 0.0
        old_cv = np.sum(old_cons)

        # self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Opt', 'to-repair'],
        #                  obj_array1=np.atleast_2d(opt.extract_obj()),
        #                  obj_array2=np.atleast_2d(to_repair.extract_obj()))

        # Perform repair mechanism (RI-2 from SACOBRA)
        x_repaired = self.repair_solution(to_repair, eps=1e-3, n_perturb=100 * self.problem.n_var, perturb_scale=10)  # eps=1e-3, n_perturb=1000, perturb_scale=3
        ...
        cv_new = np.sum(cons_arr)
        if cv_new == 0.0:
            print(colored(f'Repair Success!, cons: {repaired.extract_cons()} old CV: {old_cv:.3f}, new CV: {cv_new:.3f}', 'green'))
        else:
            print(colored(f'Repair Failed!, cons: {repaired.extract_cons()} old CV: {old_cv:.3f}, new CV: {cv_new:.3f}', 'red'))
