...
class MDE_SATLBO(EvolutionaryAlgorithm):
    ...
    def _step(self):
        ...
        exploit_population = self.surrogate_exploitation(top_ranked_population, n_population=self.n_population,
                                                            ...
                                                            maximisation_flag=self.improvement_flag)
        ...
        print('                                       ', infill_population.extract_obj().flatten())
        ...
        relative_improvement = 100 * (self.opt.obj - old_opt.obj) / old_opt.obj
        if relative_improvement <= self.improvement_threshold:
            # Update no improvement counter
            self.iter_since_improvement += len(infill_population)
        ...
        print('relative improvement (%): ', (improvement / old_opt.obj) * 100)
        print('iter_since_improvement: ', self.iter_since_improvement)
        if relative_improvement > self.improvement_threshold:
            self.iter_since_improvement = 0
            print('iter_since_improvement: reset')

        # Swap Problem to maximisation to create significant change in surrogate
        if self.iter_since_improvement >= self.max_no_improvement:
            self.improvement_flag = not self.improvement_flag
            self.iter_since_improvement = 0
            if self.improvement_flag:
                print('Switched to maximisation in surrogate_exploitation')
            else:
                print('Switched to minimisation in surrogate_exploitation')
    ...
    def surrogate_exploitation(self, init_population, n_population=30, n_generations=30, n_return=1, switch_flag=False):
