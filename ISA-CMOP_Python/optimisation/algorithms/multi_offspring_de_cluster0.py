...
class MODE(EvolutionaryAlgorithm):
    ...
    def create_surrogates(self):
        ...
        ## CLUSTER
        n_clusters = np.floor(np.linspace(self.problem.n_var, len(population), 3))  # 3 cluster sizes
        ...
        for cntr in range(len(self.current_kernel)):
            ...
            ## CLUSTER
            if n_clusters[0] > len(population):
                ...
                x_var = population.extract_var()
                obj_val = population.extract_obj()
                kmeans = KMeans(n_clusters=int(n_clusters[cntr]), max_iter=500)
                kmeans.fit(x_var)
