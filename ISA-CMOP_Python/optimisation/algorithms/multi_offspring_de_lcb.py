...
class MODE(EvolutionaryAlgorithm):
    ...
    def __init__(self,
        ...
        self.infill_chance = 0.2  ## lcb:0.25, lcb: 0.2
        ...
        self.use_both = True
        self.exploration_iterations = 5   ## lcb:20 , lcb1:5
    ...
    def _next(self):
        ...
        if not self.use_infill:
            for cntr in range(len(obj_surrogates)):
                obj_surrogates[cntr].sampling_strategy = 'lcb'
	...
	## Revert sampling strategy back to mu
        for cntr in range(len(obj_surrogates)):
            obj_surrogates[cntr].sampling_strategy = 'mu'
    ...
    def predict_infill(self, surrogate, x):
        if self.surrogate.sampling_strategy == 'ei':
            ...
            obj = surrogate.predict_lcb(x, alpha=1.866)    ## CHANGED ALPHA FROM 0.5
