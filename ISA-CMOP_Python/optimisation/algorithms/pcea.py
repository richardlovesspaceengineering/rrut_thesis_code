...
class PCEA(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        self.offspring = self.generate_offspring(self.offspring, n_gen=self.w_max, n_offspring=self.n_offspring)
        ...
        if ((self.evaluator.n_eval + 1) % self.minimise_frac) == 0:
            ...
            print(colored('----------------- Selecting most decision-diverse, feasible individual ------------------',
                          'green'))
    ...
    def generate_offspring(self, representatives, n_offspring, n_gen):
        ...
        for gen in range(n_gen - 1):
            ...
            # survived = self.conv_div_feas_survival(offspring)

            front = NonDominatedSorting().do(np.atleast_2d(offspring.extract_obj()),
        ...
        self.plot_cons_landscape(survived=survived, survived1=self.offspring_archive, block=True)

        return self.offspring_archive  # survived
    ...
    def predict_rank_sum_pof(self, offspring, delta=1e-5, gamma=1.0):
        ...
        # TODO: Progressive constraint relaxation
        if self.evaluator.n_eval < 25:
            n_cons = 2
        elif self.evaluator.n_eval < 50:
            n_cons = 3
        elif self.evaluator.n_eval < 75:
            n_cons = 4
        else:
            n_cons = 4
        feasible_order = np.argsort(-self.individual_feasible_frac)
        selected = feasible_order[:n_cons]

        # Extract from populations
        pop_x_vars = self.population.extract_var()
        off_x_vars = offspring.extract_var()
        # cons_arr = copy.deepcopy(offspring.extract_cons())
        cons_arr = copy.deepcopy(offspring.extract_cons())[:, selected]  # TODO: Progressive constraint relaxation
    ...
    def plot_cons_landscape(self, survived, survived1=None, n_grid=50, block=True):
        ...
        ax_surro = ax.contourf(x11, x22, rank_sum_pof.T, 60, cmap='Blues_r')
