...
from optimisation.operators.survival.theta_survival import cluster_association
...
class PCEA(SAEvolutionaryAlgorithm):
    ...
    def __init__(self,
        ...
        if 'survival' in kwargs:
            ...
            survival = MultipleConstraintsRankingSurvival(filter_infeasible=False, use_generalised_mcr=True)
    ...
    def _initialise(self):
        ...
        self.gamma_min = 0.02  # 0.02
        self.gamma_max = 2.0  # 2.0
        ...
        self.selected_list = list(range(len(self.ref_dirs)))
        random.shuffle(self.selected_list)

        print('Feas fracs: ', self.individual_feasible_frac)
        self.offspring = copy.deepcopy(self.population)
        self.representatives = copy.deepcopy(self.population)

        # TODO: Pearson-correlation visualisation between UPF and CPF
        # predict_pearson_correlation(self.problem, self.population)
        # self.plot_correlations(self.population)

        # Reset to normal lhs after init
        self.sampling = LatinHypercubeSampling(criterion='maximin', iterations=10000)
        self.de_mutation = DifferentialEvolutionMutation(problem=self.problem, method='diversity')
        self.infilled_pop = Population(self.problem, 0)
    ...
    def _next(self):
        ...
        self.selected_index += 1
        if self.selected_index == len(self.ref_dirs):
            self.selected_index = 0
            random.shuffle(self.selected_list)
        self.last_selected = self.selected_list[self.selected_index]
        ...
        if rand_num > 2:
            ...
            infill_population = self.phase2(self.opt, n_gen=10, n_offspring=500, n_survive=self.n_infill)
        else:
            print('Phase I - Convergence')
            # infill_population = self.phase1(self.representatives, n_gen=10, n_offspring=300, n_survive=self.n_infill)
            infill_population = self.crea_infill_stage(self.representatives, n_gen=10, n_offspring=300, n_survive=1)
        ...
        if any(np.isnan(infill_population.extract_obj())):
            print('Found infeasible point! Skipping Surrogate Update!!')
            breakpoint()
            return

        # Calculate which constraints are active
        infill_cons = infill_population.extract_cons()
        ...
        # Assign population to clusters and select most feasible per cluster
        # self.opt = self.ref_dir_survival(self.population, self.ref_dirs)

        front = NonDominatedSorting().do(np.atleast_2d(self.population.extract_obj()), return_rank=False, only_non_dominated_front=True)
        self.representatives = self.population[front]
        ...
        print(infill_population.extract_obj(), infill_population.extract_cons())
        self.update_regressor(infill_population)

        # TODO: DEBUG --------------------------------------------------------------------
        if ((self.evaluator.n_eval-1) % 200) == 0:
            self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Survived', 'Pop', 'Offs'],
                             obj_array1=self.problem.pareto_set.extract_obj(),
                             obj_array2=self.population.extract_obj(),
                             obj_array3=self.infilled_pop.extract_obj())
        # if ((self.evaluator.n_eval-1) % 50) == 0:  # or self.evaluator.n_eval == 11:
        if self.evaluator.n_eval > (self.max_f_eval - 1):
            cv_sum = self.population.extract_cons_sum()
            feas_mask = cv_sum <= 0.0
            try:
                self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Survived', 'Old', 'New'],
                                 ...
                                 obj_array3=np.atleast_2d(self.population[feas_mask].extract_obj()))
            except IndexError:
                self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Survived', 'Pop', 'Offs'],
                                 obj_array1=self.problem.pareto_set.extract_obj(),
                                 obj_array2=self.population.extract_obj(),
                                 obj_array3=self.offspring_archive.extract_obj())
        # TODO: DEBUG --------------------------------------------------------------------
    ...
    def phase1(self, representatives, n_offspring, n_gen, n_survive):
        ...
        # survived = RankAndCrowdingSurvival().do(self.problem, survived, n_survive=100)
        front = NonDominatedSorting().do(np.atleast_2d(survived.extract_obj()), only_non_dominated_front=True)
        survived = survived[front]

        comp_pop = Population.merge(copy.deepcopy(self.representatives), copy.deepcopy(self.infilled_pop[-20:]))
        min_angle = self.predict_angle_diversity(survived, comp_pop=comp_pop)
    ...
    def phase2(self, representatives, n_offspring, n_gen, n_survive):
        ...
        for gen in range(n_gen):
            # Generate offspring for specified size
            offspring = self.generate_ga_de_offspring(survived, n_offspring)

            # Evaluate predicted objective values with surrogates
            offspring = self.surrogate_evaluator.do(self.surrogate.obj_func, self.problem, offspring)

            # # TODO: Inexpensive constraints
            # real_offs = self.surrogate_evaluator.do(self.problem.obj_func, self.problem, offspring)
            # offspring.assign_cons(real_offs.extract_cons())

            # Select only the offspring assigned to the current reference direction to survive
            obj_arr = offspring.extract_obj()
            f_min, f_max = np.min(obj_arr, axis=0), np.max(obj_arr, axis=0)
            norm_obj_arr = (obj_arr - f_min) / (f_max - f_min)
            clusters, _, _ = cluster_association(norm_obj_arr, self.ref_dirs)
            selected = offspring[clusters[self.last_selected]]

            # Calculate probability of feasibility
            if self.evaluator.n_eval < (self.max_gen / 2):
                to_survive = RankAndCrowdingSurvival()._do(self.problem, selected, n_survive=50, cons_val=None)
                survived = selected[to_survive]
            else:
                pof = self.predict_rank_sum_pof(selected, gamma=self.gamma, delta=1e-4)  # 1e-5
                n_select = min(len(selected)-1, 100)
                to_survive = np.argpartition(pof, n_select)[:n_select]
                survived = selected[to_survive]
            self.offspring_archive = Population.merge(self.offspring_archive, survived)
        ...
        comp_pop = Population.merge(copy.deepcopy(self.opt), copy.deepcopy(self.infilled_pop[-10:]))
        min_angle = self.predict_angle_diversity(survived, comp_pop=comp_pop)  # , comp_pop=self.representatives
        ...
        # if self.evaluator.n_eval == 300:
        self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Pareto', 'Offs', 'Infill'],
                         obj_array1=self.problem.pareto_set.extract_obj(),
                         obj_array3=np.atleast_2d(infill.extract_obj()),
                         obj_array2=self.offspring_archive.extract_obj())

        return infill
    
    def crea_infill_stage(self, representatives, n_gen, n_offspring, n_survive):
        # Offspring Generation --------------------------------------------------------
        survived = copy.deepcopy(representatives)
        for gen in range(n_gen-1):
            # Generate offspring for specified size
        ...
        survived = self.surrogate_evaluator.do(self.surrogate.obj_func, self.problem, survived)
        # self.offspring_archive = copy.deepcopy(survived)

        # # Infill Selection ------------------------------------------------------------
        obj_arr = survived.extract_obj()
        real_obj_arr = self.representatives.extract_obj()
        ...
        # ds_metric = self.delta_s_metric(survived, self.representatives)
        # self.visualise_ds_metric()

        # Select a weighted-random individual
        rand_int = random.choices(population=list(range(len(survived))), weights=min_angle, k=n_survive)
        # rand_int = np.array([np.argmax(min_angle)])  # TODO: BIG FUCK UP
        infill_population = copy.deepcopy(survived[rand_int])

        # self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Offs', 'Reps', 'Infill'],
        #                  obj_array2=self.representatives.extract_obj(),
        #                  obj_array3=np.atleast_2d(infill_population.extract_obj()),
    ...
    def generate_ga_de_offspring(self, survived, n_offspring):
        offspring = self.mating.do(self.problem, survived, n_offspring)    # GA-SBX
        offspring = self.repair.do(self.problem, offspring)
        offspring1 = self.de_mutation.do(survived, self.opt, n_offspring)  # DE
        offspring1 = self.repair.do(self.problem, offspring1)
        offspring = Population.merge(offspring, offspring1)

        return offspring

    def delta_s_metric(self, offspring, reps):
        # Extract objectives
        rep_obj_arr = np.atleast_2d(reps.extract_obj())
        offs_obj_arr = np.atleast_2d(offspring.extract_obj())

        # Normalise objectives
        f_min = np.min(rep_obj_arr, axis=0)
        f_max = np.max(rep_obj_arr, axis=0)
        _rep_obj_arr = (rep_obj_arr - f_min) / (f_max - f_min)
        _offs_obj_arr = (offs_obj_arr - f_min) / (f_max - f_min)

        # Calculate angles between offspring and representatives -------------------------------------------------------
        # Scale by vector distances
        dist_to_ideal = np.linalg.norm(_offs_obj_arr, axis=1)
        dist_to_ideal[dist_to_ideal < 1e-64] = 1e-64
        obj_arr = _offs_obj_arr / dist_to_ideal[:, None]

        dist_to_ideal = np.linalg.norm(_rep_obj_arr, axis=1)
        dist_to_ideal[dist_to_ideal < 1e-64] = 1e-64
        real_obj_arr = _rep_obj_arr / dist_to_ideal[:, None]

        # Calculate Angle between offspring and population
        acute_angle = np.arccos(obj_arr @ real_obj_arr.T)
        closest_indices = np.argmin(acute_angle, axis=1)
        row_indices = np.arange(len(offspring))
        min_angle = acute_angle[row_indices, closest_indices]

        # Calculate PBI distances --------------------------------------------------------------------------------------
        d_ref = np.sum(_rep_obj_arr[closest_indices], axis=1)
        # pbi_d1, _ = self.calc_pbi_distances(_offs_obj_arr, _rep_obj_arr)
        # d = pbi_d1[row_indices, closest_indices]
        d = np.sum(_offs_obj_arr, axis=1)

        # Calculate convergence-diversity metric -----------------------------------------------------------------------
        ds_metric = d - d_ref + min_angle

        return ds_metric

    def visualise_ds_metric(self):
        reps_obj = self.representatives.extract_obj()
        f_min = np.min(reps_obj, axis=0)
        f_max = 1.25 * np.max(reps_obj, axis=0)

        npts = 100
        x1 = np.linspace(f_min[0], f_max[0], npts)
        x2 = np.linspace(f_min[1], f_max[1], npts)
        x11, x22 = np.meshgrid(x1, x2)
        x_mesh = np.array((x11, x22)).T

        ds_metric = np.zeros((npts, npts))
        pop = Population(self.problem, 1)
        for i in range(npts):
            for j in range(npts):
                pop.assign_obj(np.atleast_2d(x_mesh[i, j, :]))
                ds_metric[i, j] = self.delta_s_metric(pop, self.representatives)

        fig, ax = plt.subplots(figsize=(9, 7))
        ax_contour = ax.contourf(x11, x22, ds_metric.T, 100)
        ax.contour(x11, x22, ds_metric.T, [0])
        ax.scatter(reps_obj[:, 0], reps_obj[:, 1], c='k')
    ...
    def predict_rank_sum_pof(self, offspring, delta=1e-5, gamma=1.0):
        ...
        pop_x_vars = self.population.extract_var()
        off_x_vars = offspring.extract_var()
        cons_arr = copy.deepcopy(offspring.extract_cons())  # [:, 1:]

        # Use only active constraints
        active_cons = np.argwhere(self.n_active_cons > 0.0).flatten()
        if len(active_cons) > 0:
            cons_arr = cons_arr[:, active_cons]
    ...
    def predict_angle_diversity(self, population, comp_pop=None):
        obj_arr = np.atleast_2d(population.extract_obj())
        if comp_pop is None:
            real_obj_arr = np.atleast_2d(self.population.extract_obj())
        else:
            real_obj_arr = np.atleast_2d(comp_pop.extract_obj())
    ...
    def ref_dir_survival(self, population, ref_dirs):
        obj_array = population.extract_obj()
        _obj_array = (obj_array - np.min(obj_array, axis=0)) / (np.max(obj_array, axis=0) - np.min(obj_array, axis=0))
        clusters, _, _ = cluster_association(_obj_array, ref_dirs)
        survived = []
        for points in clusters:
            if len(points) == 0:
                continue
            elif len(points) == 1:
                survived.append(points[0])
            else:
                points = np.array(points)
                cv_sum = population[points].extract_cons_sum()
                survived.append(points[np.argmin(cv_sum)])
    ...
    def plot_cons_landscape(self, survived, survived1=None, n_grid=50, block=True):
        ...
        rank_sum_pof = predict_rank_sum_pof(cons_arr, idw_arr, self.problem.n_con, gamma=self.gamma, delta=1e-4)  # 1e-5
        ...
        if type(self.offspring_archive) != type(None):
            ...
            other_vars = np.atleast_2d(self.opt.extract_var())
            ax.scatter(other_vars[:, 0], other_vars[:, 1], color='k', edgecolor='k', s=60)

        if survived1 is not None:
            nondom_vars = np.atleast_2d(survived1.extract_var())
            ax.scatter(nondom_vars[:, 0], nondom_vars[:, 1], color='w', s=30)
        ax.scatter(x_vars[:, 0], x_vars[:, 1], color='r', edgecolor='w', s=50)
    ...
    def plot_correlations(self, population):
        # Extract info from population
        obj_arr = np.atleast_2d(population.extract_obj())
        var_arr = np.atleast_2d(population.extract_var())
        cons_arr = np.atleast_2d(population.extract_cons())
        cv_sum = population.extract_cons_sum()
        real_pf = np.atleast_2d(self.problem.pareto_set.extract_obj())

        # Calculate non-dominated fronts in UPF and CPF
        feasible_mask = cv_sum <= 0.0
        feas_obj_arr = copy.deepcopy(obj_arr)[feasible_mask]
        upf_front = NonDominatedSorting().do(obj_arr, only_non_dominated_front=True)
        upf_arr = obj_arr[upf_front]
        cpf_front = NonDominatedSorting().do(obj_arr, cons_val=cv_sum, only_non_dominated_front=True)
        cpf_arr = obj_arr[cpf_front]

        # upf_pop = RankAndCrowdingSurvival().do(self.problem, population, n_survive=20)
        # cpf_pop = RankAndCrowdingSurvival().do(self.problem, population, cons_val=cv_sum, n_survive=20)
        # upf_arr = upf_pop.extract_obj()
        # cpf_arr = cpf_pop.extract_obj()

        # Visualise the CPF and UPF after the LHS
        if self.problem.n_obj == 2:
            fig, ax = plt.subplots(1, 1, figsize=(9, 7))
            fig.supxlabel('Obj 1', fontsize=14)
            fig.supylabel('Obj 2', fontsize=14)

            ax.scatter(real_pf[:, 0], real_pf[:, 1], color='k', s=50, label="Real PF")
            ax.scatter(upf_arr[:, 0], upf_arr[:, 1], color='r', s=50, label="UPF")
            ax.scatter(cpf_arr[:, 0], cpf_arr[:, 1], color='b', s=25, label="CPF")
...
def predict_pearson_correlation(problem, population):
    # Extract objectives, constraints and CV sum
    obj_arr = copy.deepcopy(population.extract_obj())
    cons_arr = copy.deepcopy(population.extract_cons())
    cv_sum = copy.deepcopy(population.extract_cons_sum())

    # Normalise everything
    f_min, f_max = np.min(obj_arr, axis=0), np.max(obj_arr, axis=0)
    max_abs_cons = np.max(np.abs(cons_arr), axis=0)
    max_abs_cons[max_abs_cons == 0.0] = 1.0
    cv_max = np.max(cv_sum)
    _obj_arr = (obj_arr - f_min) / (f_max - f_min)
    _cons_arr = cons_arr / max_abs_cons
    _cv_sum = cv_sum / cv_max

    # Calculate sum of objectives
    obj_sum_arr = np.sum((1 / problem.n_obj) * _obj_arr, axis=1)
    ...
    # Extrend obj and cons arrays
    _obj_arr = np.hstack((_obj_arr, obj_sum_arr[:, None]))
    _cons_arr = np.hstack((_cons_arr, normalised_rank_sum[:, None]))

    # Calculate pearson coefficient for all combinations (n_obj + 1) x (n_cons + 1)
    n = problem.n_obj + 1
    m = problem.n_con + 1
    corr_arr = np.zeros((n, m))
    for i in range(n):
        for j in range(m):
            input_arr = np.vstack((_obj_arr[:, i].flatten(), _cons_arr[:, j].flatten()))
            corr = np.corrcoef(input_arr)
            corr_arr[i, j] = corr[0, 1]
    print(corr_arr)
    corr_arr[np.isnan(corr_arr)] = 0.0

    # visualise
    fig, ax = plt.subplots(1, 1, figsize=(9, 7))
    ax_mat = ax.matshow(corr_arr)
    # ax_mat = ax.imshow(corr_arr)
    fig.colorbar(ax_mat)
    plt.show()
