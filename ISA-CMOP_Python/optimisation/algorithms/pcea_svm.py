...
from optimisation.surrogate.classifiers.support_vector_classification import SupportVectorClassification
...
class PCEA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        # Classifier stuff
        self.classifier = SupportVectorClassification(self.problem.n_var, self.problem.x_lower, self.problem.x_upper,
                                                      C=10.0, kernel='rbf')
        self.update_classifier(self.population)
    ...
    def crea_infill_stage(self, representatives, n_gen, n_offspring, n_survive):
        ...
        for gen in range(n_gen-1):
            ...
            # Pre-filter via the constraint classifier
            feasible = self.classifier_prefilter(offspring)

            # Select promising individuals with one-by-one-selection
            fronts = NonDominatedSorting().do(feasible.extract_obj(), return_rank=False)
            survived = feasible[fronts[0]]

            self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Offs', 'Feas', 'Surv'],
                             obj_array3=np.atleast_2d(survived.extract_obj()),
                             obj_array2=np.atleast_2d(feasible.extract_obj()),
                             obj_array1=np.atleast_2d(offspring.extract_obj()))
    ...
    def classifier_prefilter(self, offspring):
        vars_arr = offspring.extract_var()
        prediction = self.classifier.predict(vars_arr)
        pred_feas_mask = prediction == 1

        return offspring[pred_feas_mask]

    def update_classifier(self, population):
        ...
        # Obtain balanced training labels using eps-relaxation
        y_labels = self.prepare_eps_balanced_training_data(cons_arr, max_abs_con_vals, balanced_target=0.3)

        # Reset classifier and train on data
        self.classifier.reset()
        self.classifier.add_points(x_vars, y_labels)
        self.classifier.train()

    @staticmethod
    def prepare_eps_balanced_training_data(y_lhs, max_abs_cons_vals, balanced_target=0.4):
        n = len(y_lhs)
        feasible_frac = 0.0
        frac = 1.0
        y_labels = np.zeros(n)
        while feasible_frac < balanced_target:
            eps = max_abs_cons_vals * (1 - frac)
            y = copy.deepcopy(y_lhs) - eps
            y[y <= 0.0] = 0.0
            y_cv = np.sum(y, axis=1)
            pos_mask = y_cv == 0.0
            y_labels[pos_mask] = 1.0
            y_labels[~pos_mask] = 0.0
            feasible_frac = np.count_nonzero(pos_mask) / n
            # print(eps, feasible_frac)
            frac *= 0.99
