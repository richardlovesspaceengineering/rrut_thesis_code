...
class PREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.hopt_frac = int(self.max_gen / 20)
        ...
        self.reps = copy.deepcopy(self.opt)

    def _next(self):
        # Conduct mating using the current population
        # self.pos = random.randint(0, len(self.w_max)-1)
        self.pos = random.choices(list(range(len(self.w_max))), weights=self.w_wins, k=1)[0]
        self.w = self.w_max[self.pos]
        self.offspring = self.generate_offspring(self.reps, n_gen=self.w, n_offspring=self.n_offspring)
        ...
        fronts = NonDominatedSorting().do(self.population.extract_obj(), only_non_dominated_front=True, return_rank=False)
        self.opt = copy.deepcopy(self.population[fronts])
        fronts = None
        self.reps = RankAndCrowdingSurvival().do(self.problem, self.population, 2 * len(self.opt), self.n_gen, self.max_gen)
    ...
    def probabilistic_infill_selection(self, offspring, optimum, n_survive=1):
        ...
        # # # TODO: Plotting debug -----------------------------------------------------------------------------------------
        # if self.evaluator.n_eval > 248 or self.evaluator.n_eval == 100:
    ...
    def calculate_hypervolume_improvement(self, population, opt):
        ...
        nadir = 1.2 * np.max(merged_obj_arr, axis=0)
