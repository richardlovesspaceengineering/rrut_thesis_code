...
from optimisation.surrogate.plog_rbf_tuner import RBFTuner
...
class PREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.f_min = None
        self.f_max = None
        self.global_x_lower = self.problem.x_lower
        self.global_x_upper = self.problem.x_upper
        self.local_x_lower = self.problem.x_lower
        self.local_x_upper = self.problem.x_upper
        self.init_d = np.linalg.norm(self.global_x_upper - self.global_x_lower)
        ...
        self.rbf_tuner = RBFTuner(n_dim=self.problem.n_var, lb=self.problem.x_lower, ub=self.problem.x_upper,
                                  c=0.5, p_type='linear', kernel_type='gaussian',
                                  width_range=(0.1, 10), k2_range=(0.1, 1e3), k1_range=(-2, 2),
                                  train_test_split=self.rbf_tuning_frac,
                                  max_evals=100, verbose=False)
        ...
        # Calculate objective ranges from the LHS
        obj_arr = self.population.extract_obj()
        f_min = np.min(obj_arr, axis=0)
        f_max = np.max(obj_arr, axis=0)
        f_range = f_max - f_min
        print("Obj ranges: ", f_range)
        # self.apply_bilog = f_range > 1e4
        self.apply_bilog = [True for _ in range(self.problem.n_obj)]  # TODO: deactive the bilog
        ...
        # TODO: generalise to any number of objectives
        if self.problem.n_obj == 2:
            self.k1 = [opt_params[0][1], opt_params[1][1]]
            self.k2 = [opt_params[0][2], opt_params[1][2]]
        else:
            self.k1 = [opt_params[0][1], opt_params[1][1], opt_params[2][1]]
            self.k2 = [opt_params[0][2], opt_params[1][2], opt_params[2][2]]

        # TODO: P-log transformation
        obj_arr = self.population.extract_obj()
        log_obj_arr = np.zeros(shape=obj_arr.shape)
        for obj_cntr, model in enumerate(self.surrogate.obj_surrogates):
            if self.apply_bilog[obj_cntr]:
                log_obj_arr[:, obj_cntr] = sp2log_transform(obj_arr[:, obj_cntr],
                                                            k1=self.k1[obj_cntr], k2=self.k2[obj_cntr])
                model.reset()
                model.add_points(self.population.extract_var(), log_obj_arr[:, obj_cntr].flatten())
                model.train()

    def _next(self):
        # Conduct mating using the current population
        self.w = self.w_max[random.randint(0, len(self.w_max)-1)]
        # reps = self.filter_within_problem_bounds(self.opt, self.local_x_lower, self.local_x_upper)
        reps = self.opt
        self.offspring = self.generate_offspring(reps, n_gen=self.w, n_offspring=self.n_offspring)
        ...
        # min_X = np.min(self.opt.extract_var(), axis=0)
        # max_X = np.max(self.opt.extract_var(), axis=0)
        print(colored(f" Opt dist / LHS dist: {np.linalg.norm(self.local_x_upper - self.local_x_lower) / self.init_d}", 'red'))
        ...
        # self.retrain_regressor(infill_population)
    ...
    def update_regressor(self, infill):
        ...
        # bilog_new_obj = bilog_transform(new_obj, beta=1)
        log_obj = np.zeros(shape=new_obj.shape)
        ...
        try:
            ...
            for obj_cntr in range(self.problem.n_obj):
                ...
                if self.apply_bilog[obj_cntr]:
                    log_obj[:, obj_cntr] = sp2log_transform(new_obj[:, obj_cntr],  k1=self.k1[obj_cntr], k2=self.k2[obj_cntr])

                    self.surrogate.obj_surrogates[obj_cntr].add_points(new_vars, log_obj[:, obj_cntr])
    ...
    def retrain_regressor(self, infill):
        # Initialise regressors
        obj_surrogates = self.surrogate.obj_surrogates

        # Create initial training data
        if len(self.population) > self.n_fixed_pop:
            # training_pop = self.population[-self.n_fixed_pop:]
            # training_pop = RankAndCrowdingSurvival().do(self.problem, self.population, self.n_fixed_pop)
            # fronts = NonDominatedSorting().do(self.population.extract_obj(), n_stop_if_ranked=self.n_fixed_pop)
            # training_pop = self.population[np.concatenate(fronts)]
            self.training_pop = self.local_training_data(self.population, self.opt)
        else:
            self.update_regressor(infill)
            return

        x_vars = self.training_pop.extract_var()
        obj_array = self.training_pop.extract_obj()
    ...
    def local_training_data(self, population, opt):
        var_arr = population.extract_var()
        pop_obj_arr = population.extract_obj()
        opt_obj_arr = np.atleast_2d(opt.extract_obj())

        # Normalise objectives by min/max from population
        f_min = np.min(pop_obj_arr, axis=0)
        f_max = np.max(pop_obj_arr, axis=0)
        pop_obj_arr = (pop_obj_arr - f_min) / (f_max - f_min)
        # opt_obj_arr = (opt_obj_arr - f_min) / (f_max - f_min)

        # Select closest via eucledian distance in objective space
        # dist_mat = np.min(distance.cdist(opt_obj_arr, pop_obj_arr), axis=1)
        dist_mat = np.sum(pop_obj_arr, axis=1)
        selected = np.argpartition(dist_mat, self.n_fixed_pop)[:self.n_fixed_pop]
        # selected = np.argsort(dist_mat)[:self.n_fixed_pop]

        # # Update local surrogate bounds
        # self.f_min = f_min + (f_max - f_min) * np.min(pop_obj_arr[selected], axis=0)
        # self.f_max = f_min + (f_max - f_min) * np.max(pop_obj_arr[selected], axis=0)

        self.local_x_lower = np.min(var_arr[selected], axis=0)
        self.local_x_upper = np.max(var_arr[selected], axis=0)

        return population[selected]
        # return RankAndCrowdingSurvival().do(self.problem, population, self.n_fixed_pop)

    def filter_within_problem_bounds(self, population, x_lower, x_upper):
        var_arr = np.atleast_2d(population.extract_var())

        within_upper = np.argwhere(np.all(var_arr < x_upper, axis=1)).flatten()
        within_lower = np.argwhere(np.all(var_arr > x_lower, axis=1)).flatten()

        in_bounds = np.unique(np.hstack((within_lower, within_upper)))

        return population[in_bounds]
    ...
    def generate_offspring(self, representatives, n_offspring=300, n_gen=10):
        if len(representatives) <= 3:
            representatives = self.population
            # if self.evaluator.n_eval > self.n_fixed_pop:
            #     representatives = self.filter_within_problem_bounds(representatives, self.local_x_lower, self.local_x_upper)

        # representatives = representatives if len(representatives) > 3 else self.population
        survived = copy.deepcopy(representatives)

        # if len(self.population) > self.n_fixed_pop:
        #     self.problem.x_lower = self.local_x_lower
        #     self.problem.x_upper = self.local_x_upper

        for gen in range(n_gen-1):
            # Generate offspring for specified size and merge with previous survivors
            # if self.evaluator.n_eval < 20 * self.problem.n_var:
            ...
            # else:
            #     offspring = self.generate_dycors_offspring(self.opt, n_offspring=n_offspring, evaluate=True)
            #     offspring = Population.merge(offspring, survived)
        ...
        # if len(self.population) > self.n_fixed_pop:
        #     self.problem.x_lower = self.global_x_lower
        #     self.problem.x_upper = self.global_x_upper
        ...
        #                  obj_array3=self.problem.pareto_set.extract_obj(),
    ...
    def generate_dycors_offspring(self, opt, n_offspring, evaluate=True):
        phi_select = 0.75
        sigma = 0.05 * (self.problem.x_upper - self.problem.x_lower)

        n = len(opt)
        m = int(n_offspring / n)
        trail_var = np.zeros((m * n, self.problem.n_var))
        cntr = 0
        for i in range(n):
            p_d = np.random.uniform(0.0, 1.0, self.problem.n_var)
            perturb_indices = np.argwhere(p_d < phi_select).flatten()
            for j in range(m):
                z = truncnorm.rvs(a=(self.problem.x_lower - opt[i].var) / sigma,
                                  b=(self.problem.x_upper - opt[i].var) / sigma,
                                  loc=opt[i].var, scale=sigma, size=self.problem.n_var)
                trail_var[cntr] = copy.deepcopy(opt[i].var)
                trail_var[cntr, perturb_indices] = z[perturb_indices]
                cntr += 1

        # Prepare offspring population
        offspring = Population(self.problem, len(trail_var))
        offspring.assign_var(self.problem, trail_var)
        offspring = self.repair.do(self.problem, offspring)
