...
from optimisation.util.misc import bilog_transform, min_prob_of_improvement
...
class PREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.apply_bilog = [False for _ in range(self.problem.n_obj)]  # TODO: deactive the bilog

        self.ks_surrogate = self.initialise_ks_regressor()

        # # TODO: Bilog Debug (retrain surrogates with bilog objectives)
        log_obj_arr = bilog_transform(self.population.extract_obj(), beta=1)
    ...
    def _next(self):
        ...
        self.offspring = self.generate_offspring(self.opt, n_gen=self.w_max[random.randint(0, len(self.w_max)-1)], n_offspring=self.n_offspring)
        ...
        self.update_ks_regressor(infill_population)
    ...
    def initialise_ks_regressor(self):
        # Create initial training data
        var_arr = self.population.extract_var()
        cons_arr = self.population.extract_cons()

        # Aggregate constraints with KS function
        ks_arr = self.predict_ks(cons_arr)
        ...
        model.add_points(var_arr, ks_arr)
        model.train()

        return model

    def update_ks_regressor(self, infill):
        # Extract update training data
        new_vars = np.atleast_2d(infill.extract_var())
        new_cons = np.atleast_2d(infill.extract_cons())

        ks_arr = self.predict_ks(new_cons)
        try:
            self.ks_surrogate.add_points(new_vars, ks_arr)
            self.ks_surrogate.train()
        except np.linalg.LinAlgError:
            print(colored(f'KS constraint regressor update failed!', 'red'))
    ...
    def update_regressor(self, infill):
        ...
        try:
            ...
            for obj_cntr in range(self.problem.n_obj):
                ...
                if self.apply_bilog[obj_cntr]:
                    self.surrogate.obj_surrogates[obj_cntr].add_points(new_vars, bilog_new_obj[:, obj_cntr])
    ...
    def generate_offspring(self, representatives, n_offspring=300, n_gen=10):
        ...
        for gen in range(n_gen-1):
            ...
            # Additional constraint for constraint KS aggregation
            var_arr = offspring.extract_var()
            obj_arr = offspring.extract_obj()
            ks_arr = np.zeros(len(offspring))
            for i in range(len(offspring)):
                ks_arr[i] = self.ks_surrogate.predict(var_arr[i])
            obj_arr = np.hstack((obj_arr, ks_arr[:, None]))
            ...
            best_front = NonDominatedSorting().do(obj_arr, only_non_dominated_front=True, return_rank=False)
    ...
    def probabilistic_infill_selection(self, offspring, optimum, n_survive=1):
        ...
        # Predict KS for offspring (as additional constraint)
        var_arr = offspring.extract_var()
        obj_arr = offspring.extract_obj()
        ks_arr = np.zeros(len(offspring))
        for i in range(len(offspring)):
            ks_arr[i] = self.ks_surrogate.predict(var_arr[i])
        print(np.min(ks_arr), np.max(ks_arr))
        obj_arr = np.hstack((obj_arr, ks_arr[:, None]))
        self.problem.n_obj += 1
        mod_offspring = Population(self.problem, len(offspring))
        mod_offspring.assign_var(self.problem, var_arr)
        mod_offspring.assign_obj(obj_arr)
        self.problem.n_obj -= 1

        # Predict KS for comparison population (as additional constraint)
        var_arr = comparison_population.extract_var()
        obj_arr = comparison_population.extract_obj()
        cons_arr = comparison_population.extract_cons()
        ks_arr = self.predict_ks(cons_arr)
        print(np.min(ks_arr), np.max(ks_arr))
        obj_arr = np.hstack((obj_arr, ks_arr[:, None]))
        self.problem.n_obj += 1
        mod_comparison_population = Population(self.problem, len(comparison_population))
        mod_comparison_population.assign_var(self.problem, var_arr)
        mod_comparison_population.assign_obj(obj_arr)
        self.problem.n_obj -= 1

        # Calculate angle diversity between offspring and the current best front
        min_angle = self.predict_angle_diversity(mod_offspring, mod_comparison_population)

        # Calculate the minimum Probability of Improvement (MPoI)
        mpoi_metric = min_prob_of_improvement(mod_offspring, mod_comparison_population)

        if any(np.isnan(mpoi_metric)):
            breakpoint()
    ...
    @staticmethod
    def predict_ks(y, rho=50):
        # Take the bilog of constraints
        y = bilog_transform(y)

        # Aggregate with KS function
        ks = (1 / rho) * np.log(np.sum(np.exp(rho * y), axis=1))
        return ks
