...
class SAMDE(EvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        ## 1. Local Minima Surrogate Mining
        optimal_population = self.local_minima_exploitation(obj_surrogates)
        ...
        # Duplicates with surrogate population
    ...
    def local_minima_exploitation(self, obj_surrogates, n_generations=100):

        # Calculate Each of the Surrogates
        optima = np.zeros((len(obj_surrogates)*self.n_infill, self.problem.n_var))
        ...
        for cntr in range(len(obj_surrogates)):
            ...
            lhs_de.do(self.n_population, x_lower=self.problem.x_lower, x_upper=self.problem.x_upper, seed=seed)
            x_var = lhs_de.x
            ...
            for iteration_counter in range(n_generations):
                ...
                if iteration_counter == 0:
                    offspring_population = self.survival.do(self.problem, offspring_population, 2*self.n_population)
                    self.n_clusters[cntr], self.cluster_centres[cntr] = self.find_optimum_n_clusters(offspring_population, obj_surrogates)
                else:
                    offspring_population = self.cluster_ranking_survival(offspring_population, self.cluster_centres[cntr], self.n_population, self.n_clusters[cntr])
                de_population = copy.deepcopy(offspring_population)

                # Update Pareto Archive
                pareto_population = self.update_archive(pareto_population, offspring_population, population_size=self.n_population)

            # Final survival for infill points
            infill_population = self.cluster_ranking_survival(de_population, self.cluster_centres[cntr], self.n_infill, self.n_clusters[cntr])
            optima[self.n_infill*cntr:self.n_infill*(cntr+1), :] = infill_population.extract_var()
    ...
    def cluster_ranking_survival(self, population, cluster_centres, n_survived, n_clusters, find_optimum=False):
        # Extract Population
        x_vars = population.extract_var()

        # Determine cluster labels from existing cluster centres without re-clustering
        # distance_matrix = distance.cdist(cluster_centres, x_vars)
        # Re-evaluate clusters with optimal cluster number
        ...
        # Conduct survival and merge populations
        survived_population = self.survival.do(self.problem, population[cluster_labels == 0], n_split[0])
        for i in range(1, n_clusters):
            try:
                dummy_pop = self.survival.do(self.problem, population[cluster_labels == i], n_split[i])
    ...
    def find_optimum_n_clusters(self, population, obj_surrogates, samples=np.array([0.2, 0.4, 0.6, 0.8]), tau=0.8):
        # Use Global Best Surrogate
        ...
        while not same_valley:
            ...
            for i in range(len(cluster_centres)):
                ...
                for j in range(len(x_cluster)):
                    ...
                    x_p = x_cluster[j, :]
                    flag[j] = self.hill_valley(x_c, x_p, surrogate, samples)

                # Accept correct cluster if 90% of individuals fit the modality
