...
class SAMDE(EvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        is_duplicate = self.check_duplications(optimal_population, self.surrogate.population, epsilon=self.duplication_tolerance)
        optimal_population = optimal_population[np.invert(is_duplicate)[0]]
    ...
    def local_minima_exploitation(self, obj_surrogates, n_generations=100):
        ...
        for cntr in range(len(obj_surrogates)):
            ...
            lhs_de.do(10*self.n_population, x_lower=self.problem.x_lower, x_upper=self.problem.x_upper, seed=seed)
            x_var = lhs_de.x
            ...
            for iteration_counter in range(n_generations):
                ...
                reference_indices = self.reference_point_selection(obj_array, self.n_population)
            ...
            if optima is not None:
                optima = np.vstack((optima, infill_population.extract_var()))
            else:
                optima = infill_population.extract_var()
