...
# TODO: Eventually will package the airfoilbenchmarksuite properly
import sys
from pathlib import Path
sys_path = str(Path('~/PycharmProjects/AirfoilBenchmarkSuite/').expanduser())
sys.path.append(sys_path)

from test_problems.stock_williams import StockWilliams
...
class SAPREA(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        self.offspring = self.generate_offspring(self.reps, n_offspring=self.n_offspring, probabilistic=False)

        # Probabilistic Infill Point Selection
        infill = self.probabilistic_infill_selection(self.offspring, n_survive=self.n_infill, probabilistic=True)
        ...
        if self.evaluator.n_eval > (self.max_f_eval - 1):
            ...
            opt_vars = self.opt.extract_var()
            opt_obj = self.opt.extract_obj()
            indices = np.argsort(opt_obj[:, 0])
            for i in range(len(indices)):
                var = opt_vars[indices[i]]
                StockWilliams()._visualise_airfoil(var, show=False)
                plt.savefig(f'/home/juan/Desktop/stock_williams_opt_airfoil{i}.png')
                plt.close()
        """

    def generate_offspring(self, representatives, n_offspring, probabilistic):
        ...
        for gen in range(self.w):
            ...
            survived = self.probabilistic_infill_selection(offspring, n_survive=int(n_offspring / 5), probabilistic=probabilistic)
    ...
    def probabilistic_infill_selection(self, offspring, n_survive=1, probabilistic=True):
        ...
        hvi = self.calculate_hypervolume_improvement(offspring, self.reps, normalise=True)
        probabilities = 2 * hvi + min_angle
        # old_prob = copy.deepcopy(probabilities)
        ...
        if probabilistic:
            rand_int = np.random.choice(list(range(len(offspring))), p=probabilities, size=n_survive, replace=False)
        else:
            rand_int = np.argpartition(-probabilities, n_survive)[:n_survive]
        infill_population = copy.deepcopy(offspring[rand_int])

        return infill_population

    def calculate_hypervolume_improvement(self, population, opt, normalise=False):
        # Extract objectives
        ...
        # Normalise hvi to [0, 1]
        if normalise:
            hvi_min = np.amin(hvi_arr)
            hvi_max = np.amax(hvi_arr)
            denom = hvi_max - hvi_min
            denom = 1.0 if denom == 0.0 else denom
            hvi_arr = (hvi_arr - hvi_min) / denom
