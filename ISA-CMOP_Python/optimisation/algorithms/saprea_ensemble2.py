...
class SAPREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.kernels = ['cubic', 'multiquadratic', 'inv_multiquadratic', 'gaussian', 'tps',
                        'linear', 'matern_12', 'matern_32', 'matern_52']
        self.n_ensemble = 3
        self.ensemble = None
        self.train_regressors(self.population)
    ...
    def _next(self):
        ...
        if (self.n_gen % self.hopt_frac) == 0:
            # Change out the kernels for each objective surrogate
            self.train_regressors(self.population)
    ...
    def train_regressors(self, population):
        var_arr = population.extract_var()
        obj_arr = population.extract_obj()

        for i in range(self.problem.n_obj):
            obj_surrogates = []
            self.ensemble = np.random.choice(self.kernels, size=self.n_ensemble, replace=False)
            print(f"Using kernels: {self.ensemble} for Obj: {i+1}")
            for k in range(self.n_ensemble):
                ...
                model.add_points(var_arr, obj_arr[:, i].flatten())
                model.train()
    ...
    def ensemble_evaluator(self, offspring):
