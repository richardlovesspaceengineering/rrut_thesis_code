...
class SATLBO(EvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.g = -2
        self.h = 6
        self.T = np.array([1])   # np.array([1, 3, 5])  ## SATLBO9 (10 infill points)

        self.n_infill_exploitation = 3
        self.n_infill_total = 10   # Change from 10 to 6 (Satlbo7)
    ...
    def _next(self):
        ...
        self.population = self.survival.do(self.problem, self.surrogate.population, self.n_population, gen=self.n_gen, max_gen=self.max_gen)
        ...
        exploration_population = self.survival.do(self.problem, temp_population, self.n_infill_total - n_eval, gen=self.n_gen, max_gen=self.max_gen)
    ...
    def teachingPhase(self, obj_surrogates, new_teacher):
        ...
        for idx in range(self.n_population):
            ...
            Tf1 = 1 + np.random.uniform(0, 1, 1)
            Tf2 = 1 + np.random.uniform(0, 1, 1)
            ...
            new_var[idx, :] = temp_var[idx, :] + a*(teacher_var - Tf1 * mean_var) + b*(teacher_var - Tf2 * temp_var[idx, :])
