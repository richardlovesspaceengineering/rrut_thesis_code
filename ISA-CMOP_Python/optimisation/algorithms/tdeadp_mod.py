...
class tDEADP(EvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        # DEBUG ------------------------------------------------------------------------------------
        # self.offspring = self.test_evaluator.do(self.problem.obj_func, self.problem, self.offspring)
        ...
        infill_population = self.one_by_one_selection(self.offspring, self.representatives, n_survive=self.n_infill)
    ...
    def generate_offspring(self, n_gen=5, n_offspring=300, flag='sbx_poly'):
        ...
        for idx in range(n_gen - 1):
            ...
            if 'sbx_poly' in flag:
                ...
                offspring = self.create_offspring.do(self.opt, survived)
                offspring = self.repair.do(self.problem, offspring)
            else:
                raise Exception(f"Offspring strategy {flag} is not recognised!")

            # Select surviving offspring
            survived = self.one_by_one_selection(offspring, offspring, n_survive=n_survive)
    ...
    def one_by_one_selection(self, offspring, representatives, n_survive=1):
        offspring = copy.deepcopy(offspring)
        # offspring = self.test_evaluator.do(self.problem.obj_func, self.problem, offspring)

        # Calculate convergence metric (EDN)
        EDN = self.preselection_strategy_two(offspring)

        # Calculate DI measure
        DI = self.calc_diversity_indicator(offspring, offspring)

        # Combine measures for non-dominated ranking
        obj_arr = np.hstack((-DI[:, None], -EDN[:, None]))
        ...
        population = copy.deepcopy(representatives)
        while len(survived) < n_survive:
            ...
            min_angles = self.calc_diversity_indicator(offspring, population)
    ...
    def calc_diversity_indicator(self, population, representatives):
        # Extract variables
        off_vars = population.extract_var()
        rep_vars = representatives.extract_var()

        # Calculate euclidean distance in the decision-space
        d_mat = distance.cdist(off_vars, rep_vars)
        if np.all(off_vars == rep_vars):
    ...
    def preselection_strategy_two(self, offspring):
