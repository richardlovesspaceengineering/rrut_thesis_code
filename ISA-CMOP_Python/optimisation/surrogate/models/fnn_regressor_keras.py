...
class DeepNeuralNetRegression(Surrogate):
    """
    ...
    def __init__(self, n_dim, l_b, u_b, activation_kernels=None, n_neurons=None, n_layers=3, n_outputs=1, **kwargs):
        ...
        if n_neurons is None:
            # self.n_neurons = np.linspace(n_dim, self.n_outputs, n_layers).astype(int)
        ...
        if 'optimizer' in kwargs:
            ...
            self.optimizer = 'adam'  # Options: 'adam', rmsprop, sgd
        ...
        self._model.add(Dense(self.n_neurons[0], input_shape=(n_dim,), activation=self.activation_kernels[0]))
        ...
        self.model = KerasRegressor(build_fn=self._baseline_model, verbose=self.verbose)
