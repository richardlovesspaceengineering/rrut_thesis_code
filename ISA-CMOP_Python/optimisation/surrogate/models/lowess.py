"""installed from scikit-lego
pip install scikit-lego
"""

import numpy as np

from sklego.linear_model import LowessRegression as Lowess

from optimisation.model.surrogate import Surrogate


class LowessRegression(Surrogate):

    def __init__(self, n_dim, l_b, u_b, span=None, sigma=0.05, **kwargs):

        super().__init__(n_dim=n_dim, l_b=l_b, u_b=u_b, **kwargs)

        self.span = span
        self.sigma = sigma
        # very sensitive to sigma. Lower sigma (0.01) gives much better approximation but leads to large oscillations
        # current default smooths quite a lot but is probably a more sensible compromise to avoid oscillations in
        # surrogate surface

        self.model = Lowess(sigma=self.sigma, span=self.span)
    ...
    def update_cv_models(self):
        ...
        self.cv_training_indices = np.array_split(random_state.choice(self.n_pts, size=self.n_pts, replace=False),
                                                  self.cv_k)
        self.cv_models = [Lowess(sigma=self.sigma, span=self.span) for _ in range(self.cv_k)]
