...
from tensorflow.keras.optimizers import RMSprop, Adagrad, Adam, Nadam
...
class NeuralNetRegression(Surrogate):
    ...
    def __init__(self, n_dim, l_b, u_b, output_dim, betas=2.0,
                 n_neurons=30,
                 batch_size=50, epochs=20000, verbose=False, patience=50, **kwargs):
        ...
        model = Sequential()
        model.add(Input(n_dim))
        model.add(Dense(self.n_neurons, activation='selu', kernel_initializer="lecun_normal"))
        model.add(Dense(self.n_neurons, activation='selu', kernel_initializer="lecun_normal"))
        ...
        # model =MLPRegressor(self.hidden, validation_fraction=0, solver='lbfgs', max_iter=self.max_iter)
