...
class RE21Setup(Setup):
    def __init__(self):
        ...
        self.ub = np.full(self.dim, 3 * tmp_val)
...
class RE22Setup(Setup):
    def __init__(self):
        ...
        self.lb = np.zeros(self.dim)
        self.lb[0] = 0.2
        self.lb[1] = 1e5*sys.float_info.min
        self.lb[2] = 1e5*sys.float_info.min
        self.ub[0] = 15
        self.ub[1] = 20
        self.ub[2] = 40
    ...
    def set_variables(self, prob, **kwargs):
        # Integer x0
        prob.add_var('x_mix', 'd', lower=self.lb[0], upper=self.ub[0],
                     # value=[14.0], scale=1.0, choices=self.feasible_vals, additional_values=[14.0])
                     value=[14.0], scale=1.0, choices=self.feasible_vals)

        # Continuous x1, x2
        prob.add_var_group('x_vars', 2, 'c', lower=self.lb[1:], upper=self.ub[1:],
                           # value=[10, 20], scale=1.0, additional_values=[[10, 20]])
                           value=[10, 20], scale=1.0)
...
class RE23Setup(Setup):
    def __init__(self):
        ...
        self.feasible_vals = np.linspace(start=self.lb[0], stop=self.ub[0], num = 1601)

        t = 1
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_mix', 2, 'd', lower=self.lb[0:2], upper=self.ub[0:2],
                           value=[38.75, 38.75], scale=1.0, choices=self.feasible_vals, additional_values=[38.75, 38.75])

        prob.add_var_group('x_vars', 2, 'c', lower=self.lb[2:], upper=self.ub[2:],
                           value=[50, 100], scale=1.0, additional_values=[[50, 100]])
...
class RE25Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_int', 1, 'i', lower=self.lb[0], upper=self.ub[0],
                           value=[45], scale=1.0, additional_values=[45])

        prob.add_var_group('x_vars', 1, 'c', lower=self.lb[1], upper=self.ub[1],
                           value=[20], scale=1.0, additional_values=[20])

        prob.add_var_group('x_mix', 1, 'd', lower=self.lb[2], upper=self.ub[2],
                           value=[0.148], scale=1.0, choices=self.feasible_vals, additional_values=[0.148])
...
class RE35Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars1', 2, 'c', lower=self.lb[0:2], upper=self.ub[0:2],
                           value=[3, 0.75], scale=1.0, additional_values=[[3, 0.75]])

        prob.add_var('x_int', 'i', lower=self.lb[2], upper=self.ub[2],
                           value=[25], scale=1.0, additional_values=[25])

        prob.add_var_group('x_vars2', 4, 'c', lower=self.lb[3:], upper=self.ub[3:],
                           value=[7.5, 7.5, 5, 5.25], scale=1.0, additional_values=[[7.5, 7.5, 5, 5.25]])
...
class RE36Setup(Setup):
    def __init__(self):
        ...
        self.lb = np.full(self.dim, 12)
        self.ub = np.full(self.dim, 60)

    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.dim, 'i', lower=self.lb, upper=self.ub,
                           value=[20], scale=1.0, additional_values = [20])
...
class RE37Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.dim, 'c', lower=self.lb, upper=self.ub,
                           value=[0.5, 0.5, 0.5, 0.5], scale=1.0)
...
class RE42Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.dim, 'c', lower=self.lb, upper=self.ub,
                           value=[200, 25, 15, 10.5, 15, 0.7], scale=1.0)
...
class CRE24Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.dim, 'c', lower=self.lb, upper=self.ub,
                           value=[3, 0.75, 20, 8, 8, 3, 4], scale=1.0)
...
class CRE25Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.dim, 'c', lower=self.lb, upper=self.ub,
                           value=25*np.ones(self.dim), scale=1.0)
...
class CRE32Setup(Setup):
    ...
    def set_variables(self, prob, **kwargs):
        prob.add_var_group('x_vars', self.dim, 'c', lower=self.lb, upper=self.ub,
                           value=[175, 25, 15, 10.5, 16, 0.65], scale=1.0)
    ...
    def obj_func(self, x_dict, **kwargs):
        ...
        obj = f
        cons = constraintFuncs
        performance = None
        return obj, cons, performance


class CRE51Setup(Setup):
    def __init__(self):
        super().__init__()
