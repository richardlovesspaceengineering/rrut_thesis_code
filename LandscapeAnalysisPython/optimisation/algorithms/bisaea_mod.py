...
class BISAEA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.minimise_frac = 10  # int(self.max_gen / (2 * self.problem.n_var))
    ...
    def minimise_regressors(self, population):
        ...
        obj_arr = (obj_arr - self.ideal) / (self.nadir - self.ideal)
        ...
        # Weighted cycling to fill out
        indices = np.array([0, 1, 2, -3, -2, 1])
        rand_ind = np.random.randint(0, len(indices), 1)

        new_arr = np.sum(obj_arr * self.ref_dirs[rand_ind], axis=1)
        print('SELECTED Weight: ', self.ref_dirs[rand_ind])
        idx = np.argmin(new_arr)
        selected = [idx]
    ...
    def one_by_one_selection(self, offspring, n_survive=1):
        ...
        if len(final_pop) > 1:
            ...
            final_pop = self.eliminate_duplicates.do(final_pop, to_itself=True)
            final_pop = self.repair.do(self.problem, final_pop)

        # Against the rest of the population
        final_pop = self.eliminate_duplicates.do(final_pop, self.population)
