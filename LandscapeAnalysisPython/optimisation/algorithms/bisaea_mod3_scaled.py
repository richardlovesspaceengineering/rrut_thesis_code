...
class BISAEA(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        if (self.n_gen % self.rbf_tuning_freq) == 0 and self.evaluator.n_eval < self.max_f_eval - 5:
    ...
    def infill_selection(self, population, n_survive=1):
        ...
        # Transform predicted fronts to be approximately always convex
        ...
        dom_ranks = np.array([]).astype(int)
        for i in range(1, len(fronts)):
            dom_ranks = np.hstack((dom_ranks, fronts[i]))
        best_arr = obj_array[fronts[0]]
        rest_arr = obj_array[dom_ranks]
        obj_arr = np.vstack((best_arr, rest_arr))

        # Find extreme points
        ext_ind = np.argmax(best_arr, axis=0)
        extremes = best_arr[ext_ind]

        try:
            # Coefficients of hyperplane
            b = np.ones(self.problem.n_obj)
            w = np.linalg.solve(extremes, b)
            d = -np.dot(w, extremes[0])

            # Signed Normal distance between points and hyperplane
            norm_d = (np.dot(best_arr, w) + d) / np.linalg.norm(w)
            scaling = ((1 - best_arr) * best_arr)
            obj_scaled = best_arr - scaling  # - 0.25 * norm_d[:, None]

            # Remaining population
            dist_mat = distance.cdist(rest_arr, best_arr)
            min_dist = np.argmin(dist_mat, axis=1)
            obj_dom_scaled = rest_arr - scaling[min_dist]  # - 0.25 * norm_d[min_dist, None]

            # Combine together
            obj_transformed = np.vstack((obj_scaled, obj_dom_scaled))

        except np.linalg.LinAlgError:
            obj_dom = obj_array
            obj_dom_scaled = obj_array
            obj_transformed = obj_array

        # obj_transformed = obj_array

        # Select minimum of weighted sum of ref_dir and predicted objective values
        weighted_obj = np.sum(obj_transformed * selected_weight, axis=1)
        ...
        #    ax.scatter(best_arr[:, 0], best_arr[:, 1], color='blue', s=50, label='original best')
        #    ax.scatter(rest_arr[:, 0], rest_arr[:, 1], color='blue', s=50, alpha=0.5, label='original rest')
        #    ax.scatter(obj_scaled[:, 0], obj_scaled[:, 1], color='orange', s=50, label='scaled best')
        #    ax.scatter(obj_dom_scaled[:, 0], obj_dom_scaled[:, 1], color='orange', alpha=0.5, s=50, label='scaled rest')
        #    ax.scatter(obj_transformed[idx, 0], obj_transformed[idx, 1], color='red', s=75, label='infill scaled')
        #    ax.scatter(obj_arr[idx, 0], obj_arr[idx, 1], color='red', s=75, label='infill original')
        #    plt.legend()
        #    plt.show()
        # TODO: DEBUG ------------------------------------------------------------------------
