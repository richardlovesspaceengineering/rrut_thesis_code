...
class CREA(EvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        # self.obj_surrogates = self.initialise_regressor()
        self.surrogate_evaluator = Evaluator()
        # self.surrogate_setup = SurrogateSetup(self.obj_surrogates)
    ...
    def _next(self):
        ...
        self.offspring = self.generate_offspring(self.representatives, n_gen=self.w_max, n_offspring=self.n_offspring,
                                                 n_reps=len(self.ref_dirs), flag=self.offspring_flag)
        ...
        if len(infill_population) == 0:
            ...
            print('----------------- Selecting from one-by-one selection! ------------------')
    ...
    def generate_offspring(self, representatives, n_offspring=300, n_gen=20, n_reps=10, flag='sbx_poly'):
        ...
        for gen in range(n_gen - 1):
            ...
            try:
                offspring1 = self.create_offspring.do(survived, self.opt)
            except:
                breakpoint()
            ...
            # survived = self.truncate_survived(offspring, fronts, n_survive=n_reps)
            # if len(fronts[0]) > n_reps:
            #     indices = np.random.choice(fronts[0], n_reps, replace=False)
            # else:
            #     indices = fronts[0]
            # survived = offspring[indices]

            # print('gen:', gen, 'n_survived:', len(survived))
    ...
    def truncate_survived(self, offspring, fronts, n_survive=10):
        front0 = fronts[0]
        front1 = fronts[1]
        survived = Population.merge(offspring[front0], offspring[front1])

        if len(front0) > n_survive:
            # If first front > n_survive, filter by most diverse points (regressor surrogates)
            survived = self.one_by_one_selection(offspring, n_survive=n_survive)

        elif len(front0) < n_survive:
            # If first & second fronts > n_survive, select highest EDN offspring (classifier surrogates)
            if len(front0) + len(front1) > n_survive:
                # Predict labels and probabilities of offspring in selected categories
                p_labels, p_proba = self.predict_offspring_pairs(survived, flag='dom1')
                t_labels, t_proba = self.predict_offspring_pairs(survived, flag='dom2')
                ...
                indices = np.argsort(-edn)[:n_survive]
                survived = survived[indices]
        else:
            # Otherwise return first front entirely
            survived = offspring[front0]
