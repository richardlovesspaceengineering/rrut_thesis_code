...
# Sundry Variables
line_colors = ['green', 'blue', 'red', 'orange', 'm', 'cyan', 'lawngreen', 'sienna',
               'gold', 'violet', 'indigo', 'cornflowerblue', 'orangered']
fill_colors = ['forestgreen', 'royalblue', 'firebrick', 'goldenrod', 'slateblue', 'turquoise', 'gold']
symbols = ['o', 'x', 's', '^', '+', '>', '<', '*', '.', '2', '3', 'd', 'p']
...
class CREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.rbf_tuner = RBFTuner(n_dim=self.problem.n_var, lb=self.problem.x_lower, ub=self.problem.x_upper,
                                  ...
                                  width_range=(0.01, 10), train_test_split=self.rbf_tuning_frac,
        ...
        self.offs = self.representatives
    ...
    def _next(self):
        ...
        self.offs = self.surrogate_evaluator.do(self.problem.obj_func, self.problem, copy.deepcopy(self.offspring))
        self.plot_pareto(ref_vec=self.ref_dirs, scaling=0.0, labels=['Exact Front', 'Representatives', 'Offspring'],
                         obj_array1=self.problem.pareto_set.extract_obj(),
                         obj_array2=self.representatives.extract_obj(),
                         obj_array3=self.offs.extract_obj(),
                         colours=['red', 'blue'], markers=['^', 'o'])
        ...
        if len(infill_population) > 0:
            try:
                ...
                print(colored('Regressor update failed!', 'red'))
        else:
            print(colored('Did not update regressor!', 'yellow'))
    ...
    def preselection_strategy_two(self, categories, n_survive=1):
        ...
        offspring = self.surrogate_evaluator.do(self.problem.obj_func, self.problem, offspring)
        ...
        if n_survive == 1:
            ...
            # Return n_survive points with highest EDN values
            infill_ind = np.argsort(-edn)[:n_survive]
        infill = offspring[infill_ind]

        self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.0, labels=['Exact Front', 'Offspring', 'Pre-selection', 'Infill Point'],
                         obj_array1=self.problem.pareto_set.extract_obj(),
                         obj_array2=self.offs.extract_obj(),
                         obj_array3=offspring.extract_obj(),
                         obj_array4=np.atleast_2d(infill.extract_obj()), selected=self.last_cluster_index,
                         colours=['blue', 'red', 'k'], markers=['o', 'p', 's'])
    ...
    @staticmethod
    def plot_pareto(ref_vec=None, obj_array1=None, obj_array2=None, obj_array3=None, obj_array4=None, fronts=None, scaling=1.5, labels=None, selected=None, colours=None, markers=None):
        ...
        font_files = mpl.font_manager.findSystemFonts(fontpaths=None,
                                                      fontext='ttf')  # fontpaths='/home/juan/Desktop/Fonts/'
        ...
        mpl.rcParams['font.family'] = 'Times New Roman'  # 'Gulliver-Regular'
        ...
        if n_obj == 2:
            ...
            plt.xlabel('Objective 1', fontsize=fontsize)
            plt.ylabel('Objective 2', fontsize=fontsize)
            ...
            if ref_vec is not None:
                if scaling > 0.0:
                    ...
                    for i in range(len(x_vec)):
                        if i == 0:
                            ax.plot(x_vec[i], y_vec[i], color='black', linewidth=0.5, label='Reference Directions')
                        else:
                            ax.plot(x_vec[i], y_vec[i], color='black', linewidth=0.5)

                    if selected is not None:
                        ax.arrow(0.0, 0.0, 1.25*ref_vec[selected, 0], 1.25*ref_vec[selected, 1], width=0.005, color='k')
            ...
            if obj_array1 is not None:
                ...
                if fronts is not None:
                    ...
                    ax.plot(obj_array1[:, 0], obj_array1[:, 1], color="black", linewidth=2, label='Exact Front')
            ...
            try:
                if obj_array2 is not None:
                    ...
                    if fronts is not None:
                        ...
                        ax.scatter(obj_array2[:, 0], obj_array2[:, 1], color=colours[0], alpha=0.75, s=50, label=labels[1], marker=markers[0])
            except:
                pass
            ...
            if obj_array3 is not None:
                ...
                if fronts is not None:
                    ...
                    ax.scatter(obj_array3[:, 0], obj_array3[:, 1], color=colours[1], alpha=1.0, s=50, label=labels[2], marker=markers[1])

            save_idx = 1
            if obj_array4 is not None:
                save_idx = 2
                obj_array4 = np.atleast_2d(obj_array4)
                ax.scatter(obj_array4[:, 0], obj_array4[:, 1], color=colours[2], alpha=1.0, s=50, label=labels[3], marker=markers[2], facecolors='w', linewidth=2)
        ...
        ax.grid(visible=True, which='major', axis='both', color='black', linewidth=0.15)
        ax.grid(visible=True, which='minor', axis='both', color='grey', linewidth=0.1)
        plt.legend(loc='upper right', frameon=False)
        plt.minorticks_on()
        # plt.tight_layout()
        # plt.show()
        plt.savefig(f'/home/juan/Desktop/crea_diagram{save_idx}.png')
