...
class CREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.rbf_tuner = RBFTuner(n_dim=self.problem.n_var, lb=self.problem.x_lower, ub=self.problem.x_upper,
                                  ...
                                  width_range=(0.01, 100), train_test_split=self.rbf_tuning_frac,
    ...
    def predict_dominance_pairs(self, offspring, representatives, flag=None):
        ...
        training_vars = np.zeros((n * m, self.problem.n_var))  # TODO: z = x - y mod
        idx = 0
        for i in range(n):
            for j in range(m):
                training_vars[idx] = (offspring[i].var - representatives[j].var)  # TODO: z = x - y mod
                idx += 1
    ...
    def predict_offspring_pairs(self, offspring, flag=None):
        # Create prediction variables
        n = len(offspring)
        training_vars = np.zeros((int(n * (n-1) / 2), self.problem.n_var))  # TODO: z = x - y mod
        idx = 0
        for i in range(n):
            for j in range(i+1, n):
                training_vars[idx] = (offspring[i].var - offspring[j].var)  # TODO: z = x - y mod
                idx += 1
    ...
    def prepare_comparison_data(self, population, domination=None):
        ...
        n = len(population)
        training_vars = np.zeros((2 * (n-1), self.problem.n_var))  # TODO: z = x - y mod
        ...
        for i in range(n-1):
            for j in range(n-1, n):
                ...
                training_vars[idx] = (population[i].var - population[j].var)  # TODO: z = x - y mod
                idx += 1

                # Assign individual [v, u] and domination label to training data
                label = dom_mat[j, i]
                training_labels[idx] = label
                training_vars[idx] = (population[j].var - population[i].var)  # TODO: z = x - y mod
                idx += 1
    ...
    def prepare_training_data(self, population, domination=None):
        ...
        # Initialise arrays
        n = len(population)
        training_vars = np.zeros((n * (n - 1), self.problem.n_var))  # TODO: z = x - y mod
