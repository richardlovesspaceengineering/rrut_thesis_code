...
class CREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.weights = [[0.0, 1.0], [0.3, 0.7], [0.5, 0.5], [0.7, 0.3], [1.0, 0.0]]
        self.weights_prob = [0.2, 0.35, 0.3, 0.1, 0.05]
    ...
    def generate_offspring(self, representatives, n_offspring=300, n_gen=20, flag='sbx_poly'):
        ...
        for gen in range(n_gen-1):
            ...
            # survived = RankAndCrowdingSurvival().do(self.problem, offspring, n_offspring)  # TODO: (crea_p_rand_rcrowd)
    ...
    def two_stage_preselection(self, offspring, population, n_survive=1, counter='seq_perm'):
        ...
        # TODO: Angle-diversity with rank-convergence ------------------------------------------------------ (crea_p_rand_div_and_conv)
        # obj_arr = offspring.extract_obj()
        #
        # # Calculate EDN number
        # dom_mat = calculate_domination_matrix(obj_arr, flag='pareto')
        # dom_mat[dom_mat != 1] = 0
        # dom_ranks = np.sum(dom_mat, axis=1)
        # dom_ranks = dom_ranks / np.max(dom_ranks)
        #
        # # Calculate Angle Diversity
        # real_obj_arr = np.atleast_2d(self.opt.extract_obj())
        #
        ...
        # min_angle = min_angle / np.max(min_angle)
        #
        # # Select most converged and angle-diverse individual
        # w1, w2 = random.choices(population=self.weights, weights=self.weights_prob, k=1)[0]
        # print('w1, w2: ', w1, w2)
        # infill_criteria = w1 * dom_ranks + w2 * min_angle
        #
        # # Select a weighted-random individual
        # rand_int = random.choices(population=list(range(len(offspring))), weights=infill_criteria, k=n_survive)
        # infill_population = copy.deepcopy(offspring[rand_int])
        #
        # # self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.0, labels=['Offs', 'Infill', 'Non-dom'],
        # #                  obj_array1=test_arr,
        # #                  obj_array2=test_arr[selected],
        # #                  obj_array3=test_arr[fronts[0]])

        # # TODO: Angle-diversity with rank-convergence ------------------------------------------------------ (crea_div_and_conv)
        # obj_arr = offspring.extract_obj()
        #
        # # Calculate EDN number
        # dom_mat = calculate_domination_matrix(obj_arr, flag='pareto')
        ...
        #
        # # Select most converged and angle-diverse individual
        # infill_criteria = 0.3 * dom_ranks + 0.7 * min_angle
        # selected = np.array([np.argmax(infill_criteria)])
        # infill_population = copy.deepcopy(offspring[selected])
        #
        # # test_arr = -np.hstack((dom_ranks[:, None], min_angle[:, None]))
        # # fronts = NonDominatedSorting().do(test_arr)
        # # fitness = np.sum(test_arr, axis=1)
        # # selected = np.array([np.argmin(fitness)])
        # # selected = fronts[0][np.random.randint(0, len(fronts[0]), 1)]
        #
        # # infill_population = copy.deepcopy(offspring[selected])
        #
        # # Select a weighted-random individual
        ...
        # TODO: Angle-diversity selection ------------------------------------------------ (crea_angle_div)
        # obj_arr = offspring.extract_obj()
        # # real_obj_arr = self.population.extract_obj()
        # real_obj_arr = np.atleast_2d(self.opt.extract_obj())
        ...
        # self.plot_pareto(ref_vec=self.ref_dirs, scaling=5.0, labels=['Offs', 'Opt', 'Infill'],
        #                  obj_array3=obj_arr[selected],
        #                  obj_array2=real_obj_arr,
        #                  obj_array1=obj_arr)

        # # TODO: Pseudo-random selection ------------------------------------------------ (crea_pseudo_rand)
        obj_arr = offspring.extract_obj()
        real_obj_arr = self.population.extract_obj()
        ...
        # TODO: Completely Random Selection ----------------------------------------------- (crea_rand)
        # if len(offspring) > 1:
        #     rand_int = np.random.randint(0, len(offspring), 1)
        #     infill_population = copy.deepcopy(offspring[rand_int])
        # elif len(offspring) == 1:
        #     infill_population = copy.deepcopy(offspring)
        # else:
        #     # Minimisation of surrogate predictions
