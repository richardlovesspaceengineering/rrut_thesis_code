...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        # Debugging Output
        print(colored(f"\t\tn_active_cons: {self.n_active_cons}\tIGD: {self.igd:.3f}\tw_max: {self.w}\tObj: {infill.extract_obj()}\tCons: {infill.extract_cons()}\tFeas: {self.feasible_frac:.3f}", 'yellow'))
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            # Create new population with active constraints only
            # active_cons = np.argwhere(self.n_active_cons > 0.0).flatten()
            active_cons = np.argpartition(-self.n_active_cons, 3)[:3]
            n_active_cons = len(active_cons)
            self.problem.n_con = n_active_cons
            mod_offspring = Population(self.problem, len(offspring))
            mod_offspring.assign_var(self.problem, offspring.extract_var())
            mod_offspring.assign_obj(offspring.extract_obj())
            mod_offspring.assign_cons(offspring.extract_cons()[:, active_cons])
