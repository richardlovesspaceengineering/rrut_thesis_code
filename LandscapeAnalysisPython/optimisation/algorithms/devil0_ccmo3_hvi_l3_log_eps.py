...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            cons_val = self.calc_cv(eps_offspring)
            if self.last_selected == 0:
                selected = self.rank_and_crowd._do(self.problem, eps_offspring, n_survive=int(n_offspring / 5), cons_val=cons_val)
            elif self.last_selected == 1:
                selected = self.two_rank_survival._do(self.problem, eps_offspring, n_survive=int(n_offspring / 5), cons_val=cons_val)
            else:
