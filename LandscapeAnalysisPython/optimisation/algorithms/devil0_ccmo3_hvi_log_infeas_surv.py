...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def survival_mechanism(self, population, filter_feasible=True):
        ...
        if not filter_feasible or feasible_frac == 0.0:
            # Find individual with lowest CV value (low-CV)
            cv_sum = self.calc_cv(population)
            lowest_cv = np.argpartition(cv_sum, 1)[:1]
            pop_var_arr = population.extract_var()
            lowest_cv_arr = pop_var_arr[lowest_cv] 

            # Conduct survival using CV vs. -dist(pop, low-CV.) non-dominated sorting
            l2_dist = -distance.cdist(np.atleast_2d(lowest_cv_arr), pop_var_arr).flatten()
            mod_obj_arr = np.hstack((l2_dist[:, None], cv_sum[:, None]))
            best_front = self.nondom.do(mod_obj_arr, only_non_dominated_front=True)
            survived = population[best_front] 
