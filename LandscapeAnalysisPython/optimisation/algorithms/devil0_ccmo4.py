...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.n_opt = 2 * self.problem.n_var
    ...
    def _next(self):
        ...
        self.offspring = self.generate_offspring(self.opt, n_offspring=self.n_offspring, n_surv=self.n_survive)
    ...
    def generate_offspring(self, representatives, n_offspring, n_surv):
        ...
        for gen in range(self.w):
            ...
            # Select most promising individuals to survive (Feasible-first)
            survived1 = self.rank_and_crowd.do(self.problem, offspring, n_survive=int(n_surv / 3))

            # Select most promising individuals to survive (Two-rank survival)
            survived2 = self.two_rank_survival.do(self.problem, offspring, n_survive=int(n_surv / 3))

            # Select most promising individuals to survive (Eps-relaxed SAFR survival)
            ...
            selected = self.safr_survival._do(self.problem, eps_population, int(n_surv / 3), cons_val=eps_population.extract_cons_sum())
            survived3 = offspring[selected]

            # Merge final survived population
            survived = Population.merge_multiple(survived1, survived2, survived3)
