...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        if self.evaluator.n_eval % 5 == 0 and self.feasible_frac == 0.0:
            cv = self.calc_cv(self.population)
            selected = np.array([np.argmin(cv)])
            lowest_cv = self.population[selected]
            repaired = self.attempt_solution_repair(lowest_cv)
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        seed_pop1 = self.calc_seed_pop(lowest_cv=self.lowest_cv, n_points=int(n_offspring / 3), frac=0.05)
        seed_pop2 = self.calc_seed_pop(lowest_cv=self.lowest_cv, n_points=int(n_offspring / 3), frac=0.1)
        seed_pop3 = self.calc_seed_pop(lowest_cv=self.lowest_cv, n_points=int(n_offspring / 3), frac=0.5)
        survived = Population.merge_multiple(survived, seed_pop1, seed_pop2, seed_pop3)
    ...
    def attempt_solution_repair(self, infill):
        # Check feasibility (do not repair if already feasible)
        cv = float(self.calc_cv(infill))
