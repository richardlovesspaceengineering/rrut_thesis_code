...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        if self.evaluator.n_eval > (self.max_f_eval - 1) or (self.evaluator.n_eval % 300 == 0):
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            # Select lowest CV individuals per ref_dir
            survived = self.select_by_partition_and_cv(offspring, ref_dirs=self.ref_dirs, n_survive=int(n_offspring / 5))
    ...
    def select_by_partition_and_cv(self, population, ref_dirs, n_survive):
        # Calculate CV sum
        ...
        obj_array = (obj_arr - f_min) / (f_max - f_min)

        # Assign clusters to population and calculate distances
        clusters, _, _ = cluster_association(obj_array, self.ref_dirs)

        # Assign objectives to ref_dirs
        survived = []
        while len(survived) < n_survive:
            for assigned in clusters:
                if len(survived) >= n_survive:
                    break

                if len(assigned) > 0:
                    cluster_cv = cv_sum[assigned]
                    to_select = np.argmin(cluster_cv)
                    survived.append(assigned[to_select])
