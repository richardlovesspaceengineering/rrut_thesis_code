...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.init_cv = float(np.max(self.calc_cv(self.population)))
        print(f'Initial max. CV: {self.init_cv:.3f}')
    ...
    def _next(self):
        ...
        repaired = self.attempt_solution_repair(infill, self.opt)

        # Update Archives
        self.population = Population.merge_multiple(self.population, infill, repaired)
        self.opt = self.survival_mechanism(self.population, filter_feasible=True)
        self.performance_indicator_and_data_storage(Population.merge(infill, repaired))
        ...
        # Surrogate Management
        if len(repaired) > 0:
            self.update_regressors(repaired)

        # Debugging Output
        cv_sum = self.calc_cv(self.population)
        cv_infill = float(self.calc_cv(infill))
        print(colored(f"min CV: {np.min(cv_sum):.3f} IGD: {self.igd:.3f} Obj: {infill.extract_obj()} Cons: {infill.extract_cons()} Feas: {self.feasible_frac:.3f} CV: {cv_infill:.3f}", 'yellow'))
    ...
    def attempt_solution_repair(self, infill, opt, cv_frac=0.05):
        # Check feasibility (do not repair if already feasible)
        cv = self.calc_cv(infill)
        if cv == 0.0 or cv > cv_frac * self.init_cv:
            # return infill
        ...
        x_repaired = self.repair_solution(infill, eps=1e-3, n_perturb=100 * self.problem.n_var, perturb_scale=20)  # eps=1e-3, n_perturb=1000, perturb_scale=3
        ...
        # TODO: Debug plot in decision space
        if self.problem.n_var == 2:
            self.plot_repair_decision_space(infill, repaired, block=True)

        # Check repair success
        cv_new = self.calc_cv(repaired)

        if cv_new == 0.0:
            print(colored(f'Repair Success! Obj: {repaired.extract_obj()} Cons: {repaired.extract_cons()} old CV: {float(cv):.3f}, new CV: {float(cv_new):.3f}', 'green'))
        else:
            print(colored(f'Repair Failed! Obj: {repaired.extract_obj()} cons: {repaired.extract_cons()} old CV: {float(cv):.3f}, new CV: {float(cv_new):.3f}', 'red'))
    ...
    def repair_solution(self, infill, eps=1e-4, perturb_scale=3, n_perturb=1000):
        ...
        for i in range(n_perturb):
            ...
            alpha = perturb_scale * np.random.uniform(0.2, 0.8, self.problem.n_con)
            r = np.sum(search_mat * alpha[:, None], axis=0)
            # print(r > self.duplication_tolerance)
            ...
            for cons_cntr in range(self.problem.n_con):
                cons[cons_cntr] = self.surrogate.cons_surrogates[cons_cntr].predict(x_candidates[i]) + self.eps[cons_cntr]
            cons[cons <= 0.0] = 0.0
    ...
    def plot_repair_decision_space(self, infill, repaired, n_grid=50, block=True):
        ...
        cons = copy.deepcopy(cons_arr)
        cons[cons <= 0.0] = 0.0
        cv_sum = np.sum(cons, axis=2)

        # Extract variables
        x_infill = np.atleast_2d(infill.extract_var())
        x_repair = np.atleast_2d(repaired.extract_var())
        x_pop = self.population.extract_var()
        
        # Plot Figure
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10, 10))
        ax_surro = ax.contourf(x11, x22, cv_sum.T, 60, cmap='Blues_r')
        ax.contour(x11, x22, cv_sum.T, [0.0001], colors='lime', linestyles='-.')
        for i in range(self.problem.n_con):
            ax.contour(x11, x22, cons_arr[:, :, i].T, [0.0], colors='red', linestyles='-.')

        ax.scatter(x_pop[:, 0], x_pop[:, 1], color='k', edgecolor='k', s=40, label='Pop')
        ax.scatter(x_infill[:, 0], x_infill[:, 1], color='m', edgecolor='w', s=60, label='Infill')
        ax.scatter(x_repair[:, 0], x_repair[:, 1], color='r', edgecolor='w', s=60, label='repaired')
