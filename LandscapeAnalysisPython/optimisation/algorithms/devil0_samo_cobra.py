...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        self.transform_list = [0 for _ in range(self.problem.n_obj + self.problem.n_con)]
    ...
    def _next(self):
        ...
        if (self.n_gen % self.hopt_frac) == 0:
            ...
            # Conduct rbf optimisation of each constraint surrogate
            ...
            for cntr, model in enumerate(opt_models):
                ...
                self.transform_list[self.problem.n_obj + cntr] = opt_params[cntr][1]
    ...
    def generate_offspring(self, representatives, n_offspring):
        ...
        for gen in range(self.w):
            ...
            if self.last_selected == 0:
                ...
                selected = self.safr_survival._do(self.problem, eps_offspring, int(n_offspring / 5), cons_val=cons_val)
    ...
    def update_regressors(self, infill):
        ...
        bilog_new_cons = bilog_transform(copy.deepcopy(new_cons), beta=1)
        ...
        for cons_cntr in range(self.problem.n_con):
            if self.transform_list[self.problem.n_obj + cons_cntr]:
                cons = bilog_new_cons[:, cons_cntr]
            else:
                cons = new_cons[:, cons_cntr]
            try:
                # Constraint surrogates
                self.surrogate.cons_surrogates[cons_cntr].add_points(new_vars, cons)
    ...
    def split_population_rbf_tuning(self):
        ...
        # test_pop = Population.merge(copy.deepcopy(self.opt), copy.deepcopy(self.infilled_pop[-4:]))
