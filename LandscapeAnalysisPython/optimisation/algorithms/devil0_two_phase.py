...
class DEVIL(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        if self.feasible_frac == 0.0:
            print('Infeasible, using hvi vs. cv_sum')
            self.offspring = self.generate_infeasible_offspring(self.opt, n_offspring=self.n_offspring)
        else:
            print('Feasible, using ccmo3')
    ...
    def generate_infeasible_offspring(self, representatives, n_offspring):
        # Select number of internal offspring generations
        self.pos = random.randint(0, len(self.w_max)-1)
        self.w = self.w_max[self.pos]
        ...
        for gen in range(self.w):
            ...
            hvi = self.calculate_hypervolume_improvement(offspring, survived)
            
            # Calculate constraint violation sum
            cons_array = copy.deepcopy(offspring.extract_cons())
            cons_array[cons_array <= 0.0] = 0.0
            cv_sum = np.sum(cons_array, axis=1)

            # Create new population with new objectives
            new_objs = np.hstack((-hvi[:, None], cv_sum[:, None]))
           
            # Conduct non-dominated survival of modified objectives
            best_front = NonDominatedSorting().do(new_objs, only_non_dominated_front=True)
            survived = offspring[best_front]

        # self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Pareto', 'Pop', 'Opt'],
        #                  # obj_array1=self.problem.pareto_set.extract_obj(),
        #                  obj_array2=new_objs,
        #                  obj_array3=new_objs[best_front])
