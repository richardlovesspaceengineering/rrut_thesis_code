...
class MDE_SATLBO(EvolutionaryAlgorithm):
    ...
    def satlbo(self, obj_surrogates, init_population, n_generations=30, n_population=30, n_survive=1):
        ...
        is_duplicate = self.check_duplications(tlbo_population, self.surrogate.population,
                                               epsilon=self.R_prox)  # Use trust region for exploration population
        tlbo_population = tlbo_population[np.invert(is_duplicate)[0]]
    ...
    def local_minima_exploitation(self, obj_surrogates, init_population, n_population=30, n_generations=50, n_survive=6, n_return=1):
        ...
        for cntr in range(len(obj_surrogates)):
            ...
            # Duplicates with the surrogate population
            is_duplicate = self.check_duplications(infill_population, self.surrogate.population, epsilon=self.duplication_tolerance) # mde_satlbo4 -> trust region in exploration
            infill_population = infill_population[np.invert(is_duplicate)[0]]
    ...
    def modal_memory_archive(self, obj_surrogates, archive_population, modal_population):
        # Initialise Archive if empty
        if len(archive_population) == 0:
            archive_population = copy.deepcopy(modal_population)
            return archive_population

        # Use Global Best Surrogate
        surrogate = obj_surrogates[-1]

        # Loop through all modal population and archive population
        selected_indices = []
        for idx in range(len(modal_population)):
            for cntr in range(len(archive_population)):
                flag = self.HVT(archive_population[cntr].var, modal_population[idx].var, surrogate)
                
                # Check if same modality
                if flag == 0:
                    # Does new point dominate old archive point
                    if modal_population[idx].obj < archive_population[cntr].obj:
                        archive_population[cntr] = modal_population[idx]
                    
                # Otherwise add to archive
                else:
                    selected_indices.append(idx)
        
        # Add new points belonging to new modalities
        archive_population = Population.merge(archive_population, modal_population[selected_indices])
