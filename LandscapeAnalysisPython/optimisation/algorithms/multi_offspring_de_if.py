...
class MODE(EvolutionaryAlgorithm):
    ...
    def __init__(self,
        ...
        ## INFLUENCE FUNCTION TRUST REGION INIT
        self.tau_scale = 0.1
        self.tau_hist = np.zeros((0, 1))
        self.theta_scale = 500
        self.theta_hist = np.zeros((0, 1))
    ...
    def _next(self):
        ...
        if self.local_and_global:
            ...
            if self.iterations_since_improvement > self.switch_after_iterations:
                self.switch_local_and_global()
                self.iterations_since_improvement = 0
        ## UPDATING THE TRUST REGION FOR THE IF FUNCTION
        improvement = self.opt.obj - old_opt
        if improvement == 0:
            self.tau_scale *= 0.97
            self.theta_scale *= 0.97
        else:
            self.tau_scale *= 1.04
            self.theta_scale *= 1.04
        # print('Tau: ', self.tau_scale)
        print('Theta: ', self.theta_scale)
        self.tau_hist = np.vstack((self.tau_hist, np.array(self.tau_scale)))
        self.theta_hist = np.vstack((self.theta_hist, np.array(self.theta_scale)))
    ...
    def create_surrogates(self):
        ...
        for cntr in range(len(self.current_kernel)):
            ...
            population_size = int(np.ceil(len(self.surrogate.population) / self.surrogate_size[0]))
            training_population = self.survival.do(self.problem, population, population_size,
                                                   gen=self.n_gen, max_gen=self.max_gen)

            x = training_population.extract_var()
            y = training_population.extract_obj()
            if self.surrogate_size[0] == 1:
    ...
    def surrogate_exploitation(self, obj_surrogates, generations=100, population_size=20,
        ...
        for cntr in range(len(obj_surrogates)):
            ...
            for cntr2 in range(len(x_init)):
                initial_objectives[cntr2, 0] = surrogate.predict(x_init[cntr2]) # + surrogate.predict_if(x_init[cntr2], theta=500)
            ...
            for iteration_counter in range(generations):
                ...
                for cntr2 in range(len(offspring_population)):
                    offspring_objectives[cntr2, 0] = surrogate.predict(offspring_variables[cntr2]) + surrogate.predict_if(offspring_variables[cntr2], theta=self.theta_scale, scale=0.2)
