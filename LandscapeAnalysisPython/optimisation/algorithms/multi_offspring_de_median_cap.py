...
class MODE(EvolutionaryAlgorithm):
    ...
    def create_surrogates(self):
        ...
        for cntr in range(len(self.current_kernel)):
            ...
            # MEDIAN CAPPING OF TRAINING POINTS BEFORE SURROGATE TRAINING
            f_median = np.percentile(y_new, 25)   # np.median(y_new)
            median_mask = y_new > f_median
            y_new[median_mask] = f_median
