...
class PCEA(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        # self.last_selected += 1
        # if self.last_selected == len(self.ref_dirs):
        ...
        if rand_num > 0.75:
            ...
            infill_population = self.phase1(self.representatives, n_offspring=300, n_gen=10, n_survive=self.n_infill)
        ...
        if self.evaluator.n_eval > (self.max_f_eval - 1) or ((self.evaluator.n_eval-1) % 200) == 0:
    ...
    def phase2(self, representatives, n_offspring, n_gen, n_survive):
        ...
        for gen in range(n_gen):
            ...
            obj_arr = copy.deepcopy(offspring.extract_obj())
            _obj_arr = obj_arr / (pof[:, None] + 1e-2)
            ...
            self.offspring_archive = Population.merge(self.offspring_archive, selected)
        ...
        #                  obj_array2=survived.extract_obj())
    ...
    def predict_rank_sum_pof(self, offspring, delta=1e-5, gamma=1.0):
        ...
        # active_cons = np.argwhere(self.n_active_cons == 0.0).flatten()  # TODO: >
        active_cons = np.array([0, 2])
        if self.evaluator.n_eval > 500:
            active_cons = np.array([0, 1, 2, 3])
