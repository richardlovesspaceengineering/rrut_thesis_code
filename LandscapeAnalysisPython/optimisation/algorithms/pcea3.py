...
class PCEA(SAEvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        self.last_selected = 22  # 10/ 22

        if self.last_selected == 2:
            # Phase I - Convergence
            print('Phase I - Convergence')
            infill_population = self.phase1(self.representatives, n_gen=self.w_max, n_offspring=300, n_survive=self.n_infill)
        # elif self.last_selected == 0 or self.last_selected == 1:
        elif self.last_selected == 10:
            # Phase II - Feasibility
            print('Phase II - Feasibility')
            infill_population = self.phase2(self.opt, n_gen=self.w_max, n_offspring=self.n_offspring,
                                            n_survive=self.n_infill)
        elif self.last_selected == 22:
            # Phase II - Feasibility
            print('Phase II - Feasibility')
            infill_population = self.phase222(self.opt, n_gen=15, n_offspring=400, n_survive=self.n_infill)
        elif self.last_selected == 0:
            # Phase II - Balanced
            print('Phase III - Balanced')
            infill_population = self.phase3(self.population, n_gen=self.w_max, n_offspring=self.n_offspring,
                                            n_survive=self.n_infill)
    ...
    def phase2(self, representatives, n_offspring, n_gen, n_survive):
        ...
        for gen in range(n_gen - 1):
            ...
            offspring = self.mating.do(self.problem, survived, n_offspring)  # GA-SBX
            offspring = self.repair.do(self.problem, offspring)
            offspring1 = self.de_mutation.do(survived, self.opt, n_offspring)
            ...
            pof = self.predict_rank_sum_pof(offspring, gamma=self.gamma, delta=1e-4)  # 1e-5
            obj_arr = offspring.extract_obj()
            _obj_arr = copy.deepcopy(obj_arr) / (pof[:, None] + 1e-2)
            offspring.assign_obj(_obj_arr)

            # Select non-dominated from feasible individuals
            # front = NonDominatedSorting().do(np.atleast_2d(obj_arr), only_non_dominated_front=True)
            # survived = offspring[front]
            survived = RankAndCrowdingSurvival().do(self.problem, offspring, n_survive=int(n_offspring / 5))
            survived = self.surrogate_evaluator.do(self.surrogate.obj_func, self.problem, survived)

            # # Filter predicted feasible-only individuals within the tau threshold
            # pof = self.predict_rank_sum_pof(survived, gamma=self.gamma, delta=1e-5)
            # survived = survived[pof > 0.9]

            # Store all-feasible only individuals in archive
            self.offspring_archive = Population.merge(self.offspring_archive, survived)
            # survived = self.offspring_archive
        ...
        n_to_select = int(0.8 * len(front))
        print(f'Offs Survived: {n_surv}, Non-dom Offs: {len(front)}')
        survived = RankAndCrowdingSurvival().do(self.problem, survived, n_survive=n_to_select)

        # Angle-Diversity in objectives calculation
        # min_dist = self.select_most_feasible_diverse(survived, return_dist_only=True)

        comp_pop = Population.merge(copy.deepcopy(self.opt), copy.deepcopy(self.infilled_pop[-5:]))
        ...
        # if self.problem.n_var == 2 and self.evaluator.n_eval > (self.max_f_eval - 2):
        # if ((self.evaluator.n_eval-1) % 25) == 0 or self.evaluator == 11:
        self.plot_cons_landscape(survived=infill, survived1=survived, block=False)

        # self.plot_pareto(ref_vec=self.ref_dirs, scaling=1.5, labels=['Pareto', 'Offs', 'Nondom'],
        #                  obj_array1=self.problem.pareto_set.extract_obj(),
        #                  obj_array2=np.atleast_2d(self.offspring_archive[np.invert(is_duplicate)[0]].extract_obj()),
        #                  obj_array3=survived.extract_obj())

        return infill

    def phase22(self, representatives, n_offspring, n_gen, n_survive):
        # Offspring Generation -----------------------------------------------------------------------------------------
        # survived = copy.deepcopy(representatives)
        self.sampling.iterations = 1000
        self.sampling.do(n_offspring, self.problem.x_lower, self.problem.x_upper, np.random.randint(0, 1e6, 1))
        ...
        for gen in range(n_gen):
            ...
            pof = self.predict_rank_sum_pof(offspring, gamma=self.gamma, delta=1e-4)  # 1e-5
            _obj_arr = copy.deepcopy(offspring.extract_obj()) / (pof[:, None] + 1e-2)
            offspring.assign_obj(_obj_arr)

            # Select non-dominated from feasible individuals
            survived = RankAndCrowdingSurvival().do(self.problem, offspring, n_survive=int(n_offspring / 2))
            survived = self.surrogate_evaluator.do(self.surrogate.obj_func, self.problem, survived)
        ...
        nondom = self.offspring_archive[front]

        # # TODO: debug plotting
        # if self.problem.n_var == 2 and self.evaluator.n_eval > (self.max_f_eval - 2):
        # if ((self.evaluator.n_eval-1) % 25) == 0 or self.evaluator == 11:
        # self.plot_cons_landscape(survived=infill, survived1=survived, block=True)
        # self.plot_cons_landscape(survived=nondom, survived1=self.offspring_archive, block=True)
    ...
    def phase222(self, representatives, n_offspring, n_gen, n_survive):
        # Offspring Generation -----------------------------------------------------------------------------------------
        # survived = copy.deepcopy(representatives)
        self.sampling.iterations = 1000
        self.sampling.do(n_offspring, self.problem.x_lower, self.problem.x_upper, np.random.randint(0, 1e6, 1))
    ...
    def phase3(self, representatives, n_offspring, n_gen, n_survive):
        ...
        for gen in range(n_gen - 1):
            # Generate offspring for specified size
            offspring = self.mating.do(self.problem, survived, n_offspring)  # GA-SBX  # TODO: survived ==> self.offspring_archive
            offspring = self.repair.do(self.problem, offspring)
            ...
            # Filter predicted feasible-only individuals within the tau threshold
            pof = self.predict_rank_sum_pof(offspring, gamma=self.gamma, delta=1e-5)
