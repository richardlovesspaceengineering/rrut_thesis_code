...
class PCEA(SAEvolutionaryAlgorithm):
    ...
    def plot_cons_landscape(self, survived, survived1=None, n_grid=50, block=True):
        ...
        rank_sum_pof = predict_rank_sum_pof(cons_arr, idw_arr, self.problem.n_con, gamma=self.gamma, delta=1e-4, eps=self.eps)  # 1e-5
...
def predict_rank_sum_pof(cons_arr, idw_arr, n_con, delta=1e-5, gamma=0.01, eps=0.):
    # Transform feasible constraint values to 0.0
    cons_arr = copy.deepcopy(cons_arr)
    cons_arr = cons_arr - eps
    cons_arr[cons_arr <= 0.0] = 0.0
