...
class SAMDE(EvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        # SAMDE Parameters
        self.explore_threshold = 0.5
        self.n_infill = 3
        self.surrogate_global_fraction = np.array([1, 1, 1])
    ...
    def _next(self):
        ...
        ## 1. Local Minima Surrogate Mining (Exploitation or Exploration based on a threshold)
        rand_int = np.random.uniform(0.0, 1.0, 1)
        if rand_int <= self.explore_threshold:
            infill_flag = 'exploit'
        else:
            infill_flag = 'explore'
        optimal_population = self.local_minima_exploitation(obj_surrogates, n_population=5*self.problem.n_var, infill_flag=infill_flag)

        # Duplicates within the local minima exploited
        is_duplicate = self.check_duplications(optimal_population, other=None, epsilon=self.duplication_tolerance)
        optimal_population = optimal_population[np.invert(is_duplicate)[0]]

        # Duplicates with the surrogate population
        is_duplicate = self.check_duplications(optimal_population, self.surrogate.population, epsilon=1e-2)
        optimal_population = optimal_population[np.invert(is_duplicate)[0]]
        ...
        print(infill_flag + ' infill: ', len(optimal_population))
    ...
    def local_minima_exploitation(self, obj_surrogates, n_population=150, n_generations=30, infill_flag='exploit'):
        ...
        for cntr in range(len(obj_surrogates)):
            ...
            for iteration_counter in range(n_generations):
                ...
                # Create Offspring
                offspring_population = self.create_offspring(de_population, pareto_population)
            ...
            infill_population = self.modal_sort_and_ranking(de_population, surrogate, self.n_infill, stop_at_cluster=3)
            infill_var = infill_population.extract_var()
            if infill_flag == 'exploit':     # Take first cluster as infill point
                if infill_var.ndim > 1:
                    infill_var = infill_var[0]
            elif infill_flag == 'explore':   # Take all but the first cluster as infill points
                if infill_var.ndim > 1:
                    infill_var = infill_var[1:]
        ...
        # Evaluate all infill points with all surrogate models and perform objective ranking for selection
        # optima_objectives = np.zeros((len(optima), len(obj_surrogates)))
        # for cntr3 in range(len(obj_surrogates)):
        #     for idx in range(len(optima)):
        #         optima_objectives[idx, cntr3] = obj_surrogates[cntr3].predict(optima[idx, :])
        #
        # # Rank the objectives
        # ranked_indices = self.rank_multiple_objectives(self, optima_objectives)
    ...
    def modal_sort_and_ranking(self, population, surrogate, n_survived, stop_at_cluster=3):
            ...
            for idx in indices:
                if not np.isnan(distances[idx]):
                    ...
                    if flag == 0:
                        ...
                        break
                        # modal_indices.append(np.argwhere(current_best == obj_ranked_var)[0])
        ...
        n_split = int(n_survived / stop_at_cluster)
        max_iter = np.min((cluster_label, stop_at_cluster))
        # n_split = [len(item) for item in np.array_split(np.arange(stop_at_cluster), n_survived)]
        ...
        for i in range(max_iter):
            ...
            n_to_survive = np.min((n_split, n_per_cluster))
    ...
    def cluster_ranking_survival(self, population, n_survived, n_clusters, find_optimum=False):
        # Extract Population
        x_vars = population.extract_var()

        # Conduct K-means clustering
        seed = np.random.randint(low=0, high=10000, size=1)
        kmeans = KMeans(n_clusters=n_clusters, random_state=seed[0])
        kmeans.fit(x_vars)
        cluster_labels = kmeans.labels_
        # cluster_centres = kmeans.cluster_centers_

        # Determine n_survived split amongst clusters
        n_split = [len(item) for item in np.array_split(np.arange(n_survived), n_clusters)]

        # Conduct survival for each cluster identified
        survived_population = None
        for i in range(n_clusters):
            # Survive n_split points from each cluster
            mask = cluster_labels == i
            if np.count_nonzero(mask) == 1:
                dummy_pop = population[mask]
            else:
                dummy_pop = self.survival.do(self.problem, population[mask], n_split[i])

            # Merge or initilise survived populations
            if survived_population is None:
                survived_population = dummy_pop
            else:
                survived_population = Population.merge(survived_population, dummy_pop)

        return survived_population

    def hill_valley(self, x_p, x_q, surrogate, samples):
        ...
        for i in range(len(samples)):
            ...
            y_init[i] = y_p + (y_q - y_p)*samples[i]

            # Determine modality using interior points
            dy = y_init[i] - surrogate.predict(x_init[i, :])
            if dy < 0.0:
                modality[i] = 1
    ...
    def find_optimum_n_clusters(self, population, surrogate, samples=np.array([0.2, 0.4, 0.6, 0.8]), tau=0.8):

        # Extract Population
        x_vars = population.extract_var()

        # Conduct Internal Iterations
        n_clusters = 2
        same_valley = False
        while not same_valley:
            # Conduct K-means clustering
            seed = np.random.randint(low=0, high=10000, size=1)
            kmeans = KMeans(n_clusters=n_clusters, random_state=seed[0])
            kmeans.fit(x_vars)
            ...
            # Check if clusters locate correctly one modality each
            modality_check = np.zeros(len(cluster_centres))
            for i in range(len(cluster_centres)):
                # Extract cluster center and individuals
                x_c = cluster_centres[i]
                x_cluster = x_vars[cluster_labels == i, :]

                # Determine modality for each individual and the cluster center
                flag = np.zeros(len(x_cluster))
                for j in range(len(x_cluster)):
                    # Calculate Hill-Valley Function to test modality
                    x_p = x_cluster[j, :]
                    flag[j] = self.HVT(x_c, x_p, surrogate, samples)

                # Accept correct cluster if X % of individuals fit the modality
                if len(flag[flag == 0]) >= np.floor(tau * len(x_cluster)):
                    modality_check[i] = 0
                else:
                    modality_check[i] = 1

            # If Tau % of modality is correct break loop, otherwise increase n_cluster by 1 and re-cluster
            if len(modality_check[modality_check == 0]) >= np.floor(tau * len(modality_check)):  # If Tau % of modality is correct
                same_valley = True
                print('Optimal n clusters: ', n_clusters)
            else:
                n_clusters += 1

        return n_clusters, cluster_centres
    ...
    def create_surrogates(self):
        ...
        for cntr in range(len(self.surrogate_global_fraction)):
            self.surrogate.obj_surrogates[0].model.kernel_type = self.kernel_list[self.current_kernel[cntr]]
            population_size = int(np.ceil(len(self.surrogate.population) / self.surrogate_global_fraction[cntr]))
            ...
            if self.surrogate_global_fraction[cntr] == 1:
