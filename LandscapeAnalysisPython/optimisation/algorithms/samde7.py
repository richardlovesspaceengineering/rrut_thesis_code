...
class SAMDE(EvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        # self.r_init = len(self.population) / self.max_gen
    ...
    def _next(self):
        ...
        ## 0.0 Adaptively resize the Trust Region R_approx
        trust_region = self.R_init_0 * (1 - len(self.surrogate.population) / (self.max_gen + self.n_init))**self.tau_r
        ...
        is_duplicate = self.check_duplications(exploit_population, self.surrogate.population, epsilon=self.R_prox)  # Use Trust Region as distance threshold
        exploit_population = exploit_population[np.invert(is_duplicate)[0]]
        ...
        explore_population = self.local_minima_exploitation(obj_surrogates, n_population=self.n_population,
                                                            n_generations=5, n_survive=2)  # Max 6 points
    ...
    def local_minima_exploitation(self, obj_surrogates, n_population=30, n_generations=5, n_survive=2):
        ...
        for cntr in range(len(obj_surrogates)):
            ...
            infill_population = self.modal_sort_and_ranking(de_population, surrogate, n_survive, stop_at_cluster=n_survive)
