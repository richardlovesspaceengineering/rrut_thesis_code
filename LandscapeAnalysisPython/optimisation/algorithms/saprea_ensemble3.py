...
class SAPREA(SAEvolutionaryAlgorithm):
    ...
    def _initialise(self):
        ...
        # TODO: Surrogate ensemble stuff
        self.ensemble = ['cubic', 'multiquadratic', 'inv_multiquadratic']
        self.n_ensemble = len(self.ensemble)

        for i in range(self.problem.n_obj):
            obj_surrogates = []
            for k in range(self.n_ensemble):
                model = RadialBasisFunctions(self.problem.n_var,
                                             l_b=self.problem.x_lower, u_b=self.problem.x_upper,
                                             c=0.5, p_type='linear', kernel_type=self.ensemble[k])
                model.add_points(self.population.extract_var(), self.population.extract_obj()[:, i].flatten())
                model.train()
                obj_surrogates.append(model)
            self.surrogate.obj_surrogates[i] = obj_surrogates
    ...
    def generate_ga_de_offspring(self, survived, n_offspring, evaluate=True):
        ...
        if evaluate:
            ...
            # TODO: Surrogate ensemble
            offspring = self.ensemble_evaluator(offspring)
        # offspring = Population.merge(offspring, survived)
    ...
    def probabilistic_infill_selection(self, offspring, n_survive=1):
        ...
        # Evaluate all ensemble surrogates
        offspring, all_obj_arr = self.ensemble_evaluator(offspring, return_evals=True)
        ...
        hvi_min, hvi_max = np.min(hvi), np.max(hvi)
        hvi = (hvi - hvi_min) / (hvi_max - hvi_min)
        hvi_arr = np.zeros((len(offspring), self.n_ensemble))
        for i in range(self.n_ensemble):
            offspring.assign_obj(all_obj_arr[:, :, i])
            hvi_arr[:, i] = self.calculate_hypervolume_improvement(offspring, self.reps)
        hvi_sigma = np.abs(np.max(hvi_arr, axis=1) - np.min(hvi_arr, axis=1))
        hvi_sigma = (hvi_sigma - np.min(hvi_sigma)) / (np.max(hvi_sigma) - np.min(hvi_sigma))
        probabilities = 2 * hvi + min_angle + 0.25 * hvi_sigma
    ...
    def update_regressors(self, infill):
        ...
        for obj_cntr in range(self.problem.n_obj):
            ...
            try:
                ...
                for k in range(self.n_ensemble):
                    self.surrogate.obj_surrogates[obj_cntr][k].add_points(new_vars, obj)
                    self.surrogate.obj_surrogates[obj_cntr][k].train()
            except np.linalg.LinAlgError:
                print(colored(f'Objective regressor {obj_cntr} update failed!', 'red'))

    def ensemble_evaluator(self, offspring, return_evals=False):
        vars = offspring.extract_var()
        obj_vars = np.zeros((len(vars), self.problem.n_obj))
        all_obj_vars = np.zeros((len(vars), self.problem.n_obj, self.n_ensemble,))
        for i in range(len(vars)):
            for j in range(self.problem.n_obj):
                objs = np.zeros(self.n_ensemble)
                for k in range(self.n_ensemble):
                    objs[k] = self.surrogate.obj_surrogates[j][k].predict(vars[i])
                all_obj_vars[i, j, :] = objs
                obj_vars[i, j] = np.mean(objs)

        offspring.assign_obj(obj_vars)

        if return_evals:
            return offspring, all_obj_vars
