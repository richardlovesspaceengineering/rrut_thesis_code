...
class SATLBO(EvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        # best_population = self.survival.do(self.problem, self.surrogate.population, self.n_lhs_split, gen=self.n_gen,
        #                                    max_gen=self.max_gen)
        ...
        for cntr in range(len(lhs_population)):
            lhs_objectives[cntr, 0] = obj_surrogates[-1].predict(temp_vars[cntr])
        ...
        temp_population = Population(self.problem, self.n_population)
        population_vars = np.vstack((ranked_population.extract_var(), lhs_population.extract_var()))
        temp_population.assign_var(self.problem, population_vars)

        # temp_objectives = np.zeros((len(temp_population), 1))
        # temp_vars = temp_population.extract_var()
        # for cntr in range(len(temp_population)):
        #     temp_objectives[cntr, 0] = obj_surrogates[-1].predict(temp_vars[cntr])
        # temp_population.assign_obj(temp_objectives)

        print(np.min(distance.cdist(lhs_population.extract_var(), ranked_population.extract_var())), self.rs_distance, len(temp_population))

        #####################################
        pop_max = np.max(self.population.extract_var(), axis=0)
        pop_min = np.min(self.population.extract_var(), axis=0)
        pop_max_dist_init = np.linalg.norm(pop_max - pop_min)
        #####################################

        for gen_idx in range(self.tlbo_max_gen):
            ## 2. Metaheuristic Exploration (1 new points)
            temp_population = self.teachingPhase(obj_surrogates, temp_population, new_teacher)
        ...
        print('Pop dist (Init, Final): ', pop_max_dist_init, pop_max_dist)
        #####################################
