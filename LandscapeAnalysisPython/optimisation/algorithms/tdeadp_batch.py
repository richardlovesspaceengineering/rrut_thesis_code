...
class tDEADP(EvolutionaryAlgorithm):
    ...
    def _next(self):
        ...
        # old_offspring = self.generate_offspring(n_gen=5, n_offspring=300)
        ...
        #                  labels=['mode', 'sbx_poly', 'Reps'])

        # Conduct two-stage pre-selection (n_infill points)
        infill_population = Population(self.problem, 0)
        for _ in range(self.n_infill):
            infill = self.two_stage_preselection(self.offspring, self.population, n_survive=1)
            infill_population = Population.merge(infill_population, infill)
        n_infill = len(infill_population)

        # Duplicate checks within infill population
        if n_infill > 1:
            infill_population = self.eliminate_duplicates.do(infill_population,
                                                             to_itself=True, return_indices=False)

        # Duplicate checks with infill population and rest of population
        infill_population = self.eliminate_duplicates.do(infill_population, self.population,
                                                         to_itself=False, return_indices=False)
        print('N infill: ', n_infill)
        ...
        if len(infill_population) == 0:
            ...
            # Select the offspring with largest distance from existing population
            infill_population = self.decision_space_diversity(self.offspring, n_survive=1, pop_to_comp=self.population)
            print('------- No infill point was found! Selecting offspring with best decision-space diversity! --------')
    ...
    def update_classifier(self, flag=None, always_update=False):
        ...
        if worst_accuracy >= self.gamma and not always_update:
    ...
    def generate_offspring(self, n_gen=5, n_offspring=300, nr=4, flag='sbx_poly'):
        ...
        for idx in range(n_gen - 1):
            ...
            # Select predicted nr * n_reps from offspring
            survived = self.preselection_strategy_two(categories, n_survive=nr * n_survive)

            # Select n_survive most spaced-out points
            survived = self.decision_space_diversity(survived, n_survive=n_survive)
    ...
    @staticmethod
    def decision_space_diversity(population, n_survive, pop_to_comp=None):
        if pop_to_comp is None:
