...
class StackingClassification(Surrogate):
    ...
    def __init__(self, n_dim, l_b, u_b, C=0.1, **kwargs):
        ...
        self.C = C
        self.model = StackingClassifier(classifiers=[LogisticRegression(penalty='l1', C=self.C),
                                                     XGBClassifier(max_depth=2),
                                                     SVC(C=10*self.C, kernel='rbf', probability=True)],
                                        use_probas=True,
    ...
    def update_cv_models(self):
        ...
        self.cv_training_indices = np.array_split(random_state.choice(self.n_pts, size=self.n_pts, replace=False),
                                                  self.cv_k)
        cv_model = StackingClassifier(classifiers=[LogisticRegression(penalty='l1', C=self.C),
                                                   XGBClassifier(max_depth=2),
                                                   SVC(C=10*self.C, kernel='rbf', gamma=0.1, probability=True)],
                                      use_probas=True, meta_classifier=LogisticRegression())
        self.cv_models = [cv_model for _ in range(self.cv_k)]
